...
| Vector store files | Durable PostgreSQL attach/list/get/update-attributes/delete lifecycle with credential-and-user isolation, durable bounded string/number/boolean attributes, source-file ownership checks, cursor pagination, atomic file-count quotas, overflow-safe per-store byte quotas, and bounded parsed-content retrieval for supported assistants files | Ingestion processing and indexing failures |
