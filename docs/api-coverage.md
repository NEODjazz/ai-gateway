...
| Chat completions | JSON, tools, structured output, vision input, SSE, generation controls, bounded multi-choice with aggregate reserve; native Groq validates citation inclusion, model-scoped reasoning effort and mutually exclusive reasoning-output controls, including format/tool compatibility, and normalizes reasoning in JSON/SSE; native DeepSeek exposes explicit thinking mode and validated effort, sampling and tool-choice compatibility while preserving the prior non-thinking default; native Cohere v2 text, structured JSON/SSE, generation controls and JSON/SSE function tools/history; native Anthropic web search with citations and actual search usage, domain-allowlisted web fetch with citations, content limits and cache exclusion, and model-specific deprecated top-k sampling; native Together preserves reasoning output/history aliases, exposes exact adjustable-effort and hybrid-disable policies by model, normalizes selected-token log probabilities and finish reasons in JSON/SSE, forwards validated sampling controls and maps both public output-token-limit names to its native field | Additional controls and model-specific policy |
| Responses | Create, indexed SSE assembly, scoped deployment affinity, owner-isolated PostgreSQL Conversations CRUD/items and serialized synchronous/background turn execution, full conversation history before policy, TPM and billing admission, function tools, MCP passthrough, bounded inline PDF input with explicit deployment capability and scanner projection, stateless reasoning history, native input-token count, owned retrieve/delete/cancel/input-items, durable background settlement, required model/input envelope checks, and range-validated generation, reasoning, text configuration and metadata controls; function and MCP tool definitions are discriminator-validated, bounded and unambiguous, and tool selection is restricted to declared targets; function and custom tool descriptions may exceed 4096 characters within the overall request-body limit; compatible transports preserve the bounded supported `include` set, detailed input/output cache and reasoning usage, provider-reported source and server-tool counters, bounded top-level citation URLs, the provider-assigned execution tier, server-side compaction, provider-side input/output moderation and validated prompt-cache controls including zero-output prewarming in JSON/SSE; DeepSeek validates supported input item and nested content-part types before execution, including provider-restored web search history, and rejects unsupported reasoning summaries and encrypted content; native Groq rejects unsupported hosted tools and limits code interpreter requests to GPT-OSS models with an automatic container, without advertising unpriced hosted execution to managed deployments; xAI preserves its server-tool item counters, uses the reported execution count in settled usage, and accepts the shared bounded `max_tool_calls` control; cache-relevant controls remain part of local cache identity, while conversation, compaction, provider-moderated and prewarm requests bypass gateway response caches and shadow execution; streaming supports explicit obfuscation control through capability-gated compatible transports | Remaining provider-specific parameters and counters |
| Response compaction | Native compact contract, bounded opaque output, model authorization, validated input and attachments, tool-history ACL, owner-checked stored screenshots resolved before content policy and usage settlement | Additional provider-native compact request options as demand is confirmed |
...
| GenerateContent | Native inbound JSON/SSE and context token counting through shared policy; owner-scoped `fileData` for stored images, PDF/plain text, signature-verifiable audio and MP4/WebM/MPEG/MPG/MOV/AVI/FLV/WMV/3GPP video, resolved after authentication through policy, quota, routing and billing; owner-scoped cached-content create/list/get/update/delete lifecycle with credential, deployment and effective-policy binding, native token counting and inference use; outbound Gemini chat/tools/vision with API-key or GCE/GKE workload authentication; native execution-tier selection and effective-tier reporting; native provider storage control with cache-safe execution; separately reported server-tool prompt tokens included in billable input with overflow-safe total validation; capability-gated per-category native safety thresholds; native Google Search grounding with validated citations, preserved search entry metadata, actual search-request billing and cache bypass; capability-gated native Python code execution with tool ACL, bounded execution history/results, streaming preservation, billing and cache bypass; native URL Context with independent capability and tool grants, validated retrieval metadata, tool-input billing and cache bypass; native Google Maps grounding with optional validated coordinates, bounded place/review metadata, citations, actual-use billing and cache bypass; bounded signature-validated inline WAV, MP3/MPEG, AIFF, AAC, OGG/Opus, FLAC, M4A and WebM audio with AV projection, explicit capability routing and cache bypass; Vertex-only timestamp-aware audio processing with explicit deployment capability; validated global and per-part media token-resolution controls, including stored media references and native context counting for Gemini and Vertex; capability-gated static or agentic per-video processing with stored-reference preservation; RFC3339-bounded native Google Search with explicit Gemini/Vertex capability routing; native retrieval from provider-managed file-search stores with per-store ACL, token reserve/counting, validated grounding metadata, explicit Gemini/Vertex capability routing and response-cache bypass; native client-executed browser/mobile/desktop computer-use actions with environment and safety-override ACL, prompt-injection detection controls, token counting and cache bypass; native remote MCP through registry IDs with explicit provider-execution opt-in, connector ACL, token counting and cache bypass | Provider-specific remote MCP transports other than Streamable HTTP |
...
| Apply guardrail | Authenticated public execution with attached-policy authorization, access groups, RPM/TPM, fail-closed durable audit, bounded DLP/AV scans, configured anonymization and metadata-only monitoring; anonymization-only policies are supported, enabled modules must be available, and responses expose only replacement counts plus masked text when a replacement occurred | Additional scanner protocols when justified by configured policy needs |
...
| Vector store files | Durable PostgreSQL attach/list/get/update-attributes/delete lifecycle with credential-and-user isolation, durable bounded string/number/boolean attributes, source-file ownership checks, bidirectional cursor pagination with timestamp order and status filtering, atomic file-count quotas, overflow-safe per-store byte quotas, bounded parsed-content retrieval, and persisted auto/static conservative-token chunking with bounded overlap | Ingestion processing and indexing failures |
| Vector store file batches | Atomic durable attachment of 1 to 2000 owned files, global or per-file attributes, persisted auto/static chunking policy, aggregate count/byte quotas, owner-scoped retrieval, idempotent terminal cancellation and bidirectional file pagination | Background ingestion and partial per-file processing states |
| Vector store search | Owner-scoped synchronous semantic search for stores with up to 20 UTF-8 text/Markdown/CSV/JSON attachments and 1 MiB matched content; bounded exact, equality, membership, numeric-comparison and nested `and`/`or` attribute filters run before content reads; persisted-strategy Unicode-safe chunking, model authorization, effective content policy, TPM/budget reserve, one batched embeddings call, exact provider usage settlement and validated cosine ranking | Durable ingestion/indexing and larger stores |
...
| --- | --- | --- |
| Native provider catalog | Anthropic, Ollama, Gemini, Cohere Chat/Rerank/Embeddings, Mistral Chat/Embeddings/FIM, Voyage Embeddings/Rerank, Bedrock Chat, Groq with model-scoped reasoning effort for Chat and Responses, DeepSeek with Flash-only image input and the `apply_patch` Responses custom tool, Cerebras Chat with model-scoped thinking-history control, NVIDIA NIM Chat/Messages/count-tokens/Completions/Responses create/retrieve/cancel/Embeddings/Rerank, Together Chat/Completions/Embeddings/Rerank/Image Generation/Transcription/Translation/Text-to-Speech and xAI Chat/Responses/Embeddings with model-scoped reasoning effort, including the distinct multi-agent Responses behavior; provider-specific operations have protocol tests; the admin capability profile reports adapter operations, deployment capabilities, authentication modes and validated parameter support separately, including exact model overrides for Chat and Responses | Additional provider adapters with protocol tests; extend the request-parameter matrix as controls are added |
| Azure | Native resource-root `/openai/v1`, Foundry project `/api/projects/{project}/openai/v1`, explicit deployment paths and versioned resource-root routing for managed and startup-configured deployments, API version forwarding, API-key and Entra bearer authentication, public/US Government/China authority and resource audience selection, explicit Cognitive Services or Foundry Entra audience override, environment-backed service-principal client credentials, ambient AKS federation and App Service/Container Apps/VM managed identity with refresh, discovery and shared inference lifecycle; Realtime WebSocket uses the native GA model query or preview deployment/API-version query and the same credential chain; web search is isolated to Foundry project Responses and rejected on Azure resource endpoints and Chat | Additional sovereign-cloud contracts after provider availability is confirmed |
...
- `7765869`: Native Together Chat maps the provider's `eos` terminal reason to
  public `stop`, preserves the other documented reasons and rejects malformed,
  missing or unknown terminal values in JSON and SSE.
