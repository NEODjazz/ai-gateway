...
| Responses | Create, indexed SSE assembly, scoped deployment affinity, function tools, MCP passthrough, bounded inline PDF input with explicit deployment capability and scanner projection, stateless reasoning history, native input-token count, owned retrieve/delete/cancel/input-items, durable background settlement, range-validated generation options and metadata; compatible transports preserve the bounded supported `include` set plus validated prompt-cache mode, TTL, comparison response and retention controls in JSON/SSE and local cache identity; streaming supports explicit obfuscation control through capability-gated compatible transports | Remaining provider-specific parameters and counters |
