...
| Responses | Create, indexed SSE assembly, scoped deployment affinity, owner-isolated PostgreSQL Conversations CRUD/items and serialized turn execution, full conversation history before policy, TPM and billing admission, function tools, MCP passthrough, bounded inline PDF input with explicit deployment capability and scanner projection, stateless reasoning history, native input-token count, owned retrieve/delete/cancel/input-items, durable background settlement, required model/input envelope checks, and range-validated generation, reasoning, text configuration and metadata controls; function and MCP tool definitions are discriminator-validated, bounded and unambiguous, and tool selection is restricted to declared targets; compatible transports preserve the bounded supported `include` set, server-side compaction, provider-side input/output moderation and validated prompt-cache controls including zero-output prewarming in JSON/SSE; cache-relevant controls remain part of local cache identity, while conversation, compaction, provider-moderated and prewarm requests bypass gateway response caches and shadow execution; streaming supports explicit obfuscation control through capability-gated compatible transports | Background execution within a conversation and remaining provider-specific parameters and counters |
