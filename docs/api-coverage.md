...
| Assistants | Durable assistant definition CRUD/list; thread and message lifecycle; background run create/list/get/cancel with provider routing, policy enforcement, billing settlement, owner isolation, active-run serialization, bounded retention and directional pagination | Run steps, tool-output continuation and built-in assistant tools |
