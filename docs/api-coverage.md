...
| Batches | Durable JSONL jobs for Chat, Responses, response compaction, text completions, embeddings, rerank, search, image generation, text to speech, OCR and moderation; speech audio is stored in an explicit bounded base64 JSON envelope; OCR file references are resolved to owner-scoped content snapshots before queueing; owned input/output/error files, bounded asynchronous workers, per-item execution IDs, policy snapshots, token, character, page and search-unit accounting, quotas, cancellation, expiry and idempotent settlement | Additional batchable endpoint families and provider-native batch transports when required |
