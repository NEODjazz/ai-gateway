...
| MCP | Registry, connector/tool-specific ACL, policy-filtered discovery, Responses passthrough, encrypted static server bearer credentials, bounded Streamable HTTP execution, replica-local bounded session reuse, durable PostgreSQL idempotency, fail-closed audit and exact tool-request settlement | Additional credential schemes and transports |
