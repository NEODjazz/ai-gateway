...
| Bedrock Invoke | Synchronous `/model/{model}/invoke` for the bounded Anthropic Messages dialect through shared authentication, function-tool policy, quota, retry and exact billing; explicit deployment capability, bearer or AWS SigV4 authentication, strict request and response validation, bounded transport and provider usage conversion | Additional model dialects and streaming InvokeModel only when required |
