...
| Assistants | Durable assistant definition CRUD/list; thread and message lifecycle; background run create/list/get/cancel and exact-set function tool-output continuation; successful output, run status and message-creation step commit atomically after billing settlement. All resources use owner isolation, quotas, bounded retention and directional pagination | Built-in assistant tool execution |
