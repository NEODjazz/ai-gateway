...
| Vector store files | Durable PostgreSQL attach/list/get/update-attributes/delete lifecycle with credential-and-user isolation, durable bounded attributes, source-file ownership checks, cursor pagination, atomic file-count quotas and overflow-safe per-store byte quotas | Ingestion processing and indexing failures |
