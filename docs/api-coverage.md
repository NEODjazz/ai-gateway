...
| RAG query | Owner-scoped semantic retrieval with optional reranking and Chat generation; each stage independently enforces authorization, content policy, quotas, routing and billing, with JSON and SSE output; synchronous JSON maps model-written source markers to owner-scoped file citation annotations | Durable precomputed indexes, streaming citation annotations, claim verification and larger retrieval corpora |
