...
| Assistants | Durable assistant definition CRUD/list; thread and message lifecycle; background run create/list/get/cancel and exact-set function tool-output continuation with provider routing, policy enforcement, billing settlement, owner isolation, active-run serialization, bounded retention and directional pagination | Run steps and built-in assistant tools |
