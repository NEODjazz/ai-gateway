...
| GenerateContent | Native inbound JSON/SSE and context token counting through shared policy; outbound Gemini chat/tools/vision with API-key or GCE/GKE workload authentication; capability-gated per-category native safety thresholds with cache bypass | Cached content, grounding/server tools, file/audio parts and additional native options |
