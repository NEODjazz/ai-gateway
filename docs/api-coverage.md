...
| A2A | A2A 1.0 profile-scoped Agent Cards and synchronous text `SendMessage` through shared authentication, model policy, quotas, guardrails, routing and billing; PostgreSQL-backed completed tasks with credential-and-user ownership, TTL, atomic quota, `GetTask`, filtered/cursor-paginated `ListTasks`, bounded history, synchronous continuation with optimistic concurrency and explicit terminal cancellation semantics | Asynchronous execution, streaming, push notifications, extended cards and non-text parts |
...
video jobs, asynchronous A2A task execution and an owned sandbox runtime. Further parameter additions alone cannot
