...
| Bedrock Converse | Synchronous `/model/{model}/converse` through shared authorization, tool policy, quota, retry and exact billing; native text/function transport, bounded base64 user images and documents with DLP/AV projection and document-aware TPM reserve, strict usage validation, separately attributed requests and AWS workload credentials | Event stream and additional native content blocks |
