...
| MCP | Registry, connector/tool-specific ACL, policy-filtered discovery, Responses passthrough, encrypted static server bearer credentials, bounded Streamable HTTP execution with durable PostgreSQL idempotency, fail-closed audit and exact tool-request settlement | Additional credential schemes, transports and long-lived session reuse |
