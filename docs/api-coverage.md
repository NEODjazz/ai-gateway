...
| Assistants | Durable assistant definition CRUD/list; thread CRUD; message CRUD/list with directional cursor pagination, multimodal content and owned attachments. Resources use credential-and-user ownership, bounded schemas, atomic quotas and optimistic updates | Initial thread messages, run, run-step and tool-output execution lifecycle |
