...
| Audio transcription | Authenticated `/v1/audio/transcriptions`, bounded multipart audio, extension/MIME/signature validation, prompt/keyword DLP, file AV projection, bounded multilingual hints and chunking strategy, explicit capability routing, compatible/Azure transport and exact token settlement | Streaming, duration-priced models, known-speaker references and additional native provider adapters |
