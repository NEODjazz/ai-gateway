...
| Interactions | Synchronous `/v1/interactions` mapped through the Responses authorization, tool policy, quota, affinity, retry and exact billing lifecycle; string/step input, function tools, structured output, continuity and bounded generation controls; separately attributed usage | Streaming, background jobs, agents and provider-native controls |
...
| Azure | Native resource-root `/openai/v1` and explicit deployment paths, API version forwarding, API-key and Entra bearer authentication, ambient AKS federation and App Service/Container Apps/VM managed identity with refresh, discovery and shared inference lifecycle | Sovereign-cloud authority and resource audiences |
| Workload identity | Bedrock SigV4 supports encrypted explicit credentials, environment keys, bounded shared credentials profiles, regional web-identity STS exchange, refreshable ECS/EKS container roles and EC2 IMDSv2 instance roles; Azure Entra supports AKS projected-token federation, local managed-identity endpoints and IMDS; Gemini supports short-lived GCE/GKE metadata tokens | Sovereign Azure clouds and additional federated profile types when justified |
