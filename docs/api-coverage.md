...
| Batches | Durable JSONL jobs for Chat, Responses, text completions, embeddings, rerank, search, image generation and moderation; owned input/output/error files, bounded asynchronous workers, per-item execution IDs, policy snapshots, token and search-unit accounting, quotas, cancellation, expiry and idempotent settlement | Additional batchable endpoint families and provider-native batch transports when required |
