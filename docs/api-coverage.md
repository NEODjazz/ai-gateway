...
| Batches | Durable JSONL jobs for Chat, Responses, text completions, embeddings, rerank, search, image generation, text to speech and moderation; speech audio is stored in an explicit bounded base64 JSON envelope; owned input/output/error files, bounded asynchronous workers, per-item execution IDs, policy snapshots, token, character and search-unit accounting, quotas, cancellation, expiry and idempotent settlement | Additional batchable endpoint families and provider-native batch transports when required |
