...
| `deepseek` | Chat/stream and Responses with provider-specific validation, reasoning history passthrough, explicit thinking-mode control and validated reasoning effort; omission keeps the gateway's non-thinking default; `top_p` is rejected when thinking is disabled because the provider ignores it |
