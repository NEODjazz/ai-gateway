...
| `deepseek` | Chat/stream and Responses with provider-specific validation, reasoning history passthrough, explicit Chat thinking-mode control and validated reasoning effort; omission keeps the gateway's non-thinking Chat default; `top_p` is rejected when Chat thinking is disabled because the provider ignores it; Responses accepts `reasoning.effort=none` to disable thinking and `minimal` for provider-compatible low effort |
