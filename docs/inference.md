...
| `xai` | Chat/stream, Responses and Embeddings with bearer authentication, merged text/embedding model discovery, structured output, vision, web search, response compaction and owned retrieve/input-items/delete lifecycle; priority tier, bounded reasoning/logprobs and Responses execution-control validation with response preservation, float/base64 vectors, exact embedding token usage and validated server-side tool item counters |
