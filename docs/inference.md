...
| `ollama` | Native chat/stream/embeddings и provider completions JSON/SSE для строкового prompt; native `top_k`, `min_p`, log probabilities и reasoning history/output; optional bearer credential for remote inference. Completions отклоняет `best_of`, `echo`, `logit_bias`, `n` и `user`, которые upstream не применяет |
