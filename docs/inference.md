...
| `nvidia-nim` | Chat/stream, native Messages/stream and count-tokens, legacy Completions, Responses and Embeddings with optional bearer authentication and model discovery; Chat and Messages use isolated cache scopes, and model-dependent multimodal input is enabled per deployment |
