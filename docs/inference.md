...
| `xai` | Chat/stream, Responses and Embeddings with bearer authentication, merged text/embedding model discovery, structured output, vision, web search, response compaction and owned retrieve/input-items/delete lifecycle; priority tier, bounded reasoning/logprobs, Responses execution controls, lifecycle, isolation, prompt-cache, moderation results and deanonymized instruction metadata, effective generation settings and strict echoed tool configuration with response preservation, float/base64 vectors, exact embedding token usage and validated server-side tool item counters |
