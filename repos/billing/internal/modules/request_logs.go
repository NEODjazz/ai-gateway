...
func (r *ClickHouseUsageReporter) ListRequestLogGroups(ctx context.Context, filter RequestLogGroupFilter) (RequestLogGroupPage, error) {
	...
	query := fmt.Sprintf(`SELECT group_id,requests,errors,models,providers,total_tokens,training_tokens,input_characters,input_pages,input_audio_milliseconds,video_seconds,tool_requests,cache_read_input_tokens,cache_write_input_tokens,search_requests,cache_hits,latency_ms,cost,currency,started_at,ended_at
FROM (
