...
func TestClickHouseUsageReporterBuildsBoundedReport(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.Contains(query, "sum(training_tokens) AS training_tokens") || !strings.Contains(query, "sum(video_seconds) AS video_seconds") || !strings.Contains(query, "sum(output_images) AS output_images") || !strings.Contains(query, "{from:UInt64}") || !strings.Contains(query, "{to:UInt64}") || r.URL.Query().Get("param_from") != "1785067200" || r.URL.Query().Get("param_to") != "1787659200" || !strings.Contains(query, "safe_db.safe_events AS usage") || !strings.Contains(query, "provider_id != ''") || !strings.Contains(query, "substring(pricing_key, position(pricing_key, '/') + 1)") || !strings.Contains(query, "startsWith(model, concat(provider_endpoint_name, '-'))") || !strings.Contains(query, "'upstream_model' AS kind") || !strings.Contains(query, "if(upstream_model = '', 'Unassigned', upstream_model)") || !strings.Contains(query, "'endpoint' AS kind") || !strings.Contains(query, "if(provider_endpoint_name = '', 'Unassigned', provider_endpoint_name)") || !strings.Contains(query, "sum(cache_read_input_tokens) AS cache_read_input_tokens") || !strings.Contains(query, "sum(cache_write_input_tokens) AS cache_write_input_tokens") || !strings.Contains(query, "sum(search_requests) AS search_requests") || !strings.Contains(query, "sum(input_characters) AS input_characters") || !strings.Contains(query, "sum(input_pages) AS input_pages") || !strings.Contains(query, "sum(input_audio_milliseconds) AS input_audio_milliseconds") || !strings.Contains(query, "sum(tool_requests) AS tool_requests") || !strings.Contains(query, "countIf(cache_status = 'hit')") || !strings.Contains(query, "ARRAY JOIN if(empty(tags), ['Untagged'], tags)") || !strings.Contains(query, "'key' AS kind") || !strings.Contains(query, "'user' AS kind") || !strings.Contains(query, "'team' AS kind") || !strings.Contains(query, "'organization' AS kind") || !strings.Contains(query, "if(api_key_fingerprint = '', 'Unassigned', api_key_fingerprint)") || !strings.Contains(query, "sum(usage.cost) AS cost") || strings.Contains(query, "sum(cost) / count()") || strings.Contains(query, "token_hash") {
			t.Fatalf("unsafe report query: %s", query)
		}
		_, _ = w.Write([]byte("{\"kind\":\"total\",\"date\":\"\",\"name\":\"\",\"currency\":\"USD\",\"requests\":2,\"errors\":1,\"input_tokens\":10,\"output_tokens\":5,\"training_tokens\":1000,\"total_tokens\":1015,\"input_characters\":4096,\"input_pages\":4,\"input_audio_milliseconds\":90000,\"video_seconds\":12,\"tool_requests\":3,\"cache_read_input_tokens\":6,\"cache_write_input_tokens\":2,\"search_requests\":2,\"cost\":0.25,\"avg_latency_ms\":12.5}\n" +
	...
	if err != nil || len(report.Totals) != 1 || report.Totals[0].TotalTokens != 1015 || report.Totals[0].TrainingTokens != 1000 || report.Totals[0].InputCharacters != 4096 || report.Totals[0].InputPages != 4 || report.Totals[0].InputAudioMilliseconds != 90000 || report.Totals[0].VideoSeconds != 12 || report.Totals[0].ToolRequests != 3 || report.Totals[0].CacheReadInputTokens != 6 || report.Totals[0].CacheWriteInputTokens != 2 || report.Totals[0].SearchRequests != 2 || len(report.ByModel) != 1 || len(report.ByUpstreamModel) != 1 || report.ByUpstreamModel[0].Name != "gpt-5.6-2026-07-09" || len(report.ByEndpoint) != 1 || report.ByEndpoint[0].Name != "azure-primary" || len(report.ByTag) != 1 || report.ByTag[0].Name != "production" || len(report.ByKey) != 1 || report.ByKey[0].Name != "key-1" || len(report.ByUser) != 1 || len(report.ByTeam) != 1 || len(report.ByOrganization) != 1 {
