...
func TestClickHouseUsageReporterBuildsBoundedReport(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.Contains(query, "{from:UInt64}") || !strings.Contains(query, "{to:UInt64}") || r.URL.Query().Get("param_from") != "1785067200" || r.URL.Query().Get("param_to") != "1787659200" || !strings.Contains(query, "safe_db.safe_events AS usage") || !strings.Contains(query, "provider_id != ''") || !strings.Contains(query, "substring(pricing_key, position(pricing_key, '/') + 1)") || !strings.Contains(query, "startsWith(model, concat(provider_endpoint_name, '-'))") || !strings.Contains(query, "'upstream_model' AS kind") || !strings.Contains(query, "if(upstream_model = '', 'Unassigned', upstream_model)") || !strings.Contains(query, "'endpoint' AS kind") || !strings.Contains(query, "if(provider_endpoint_name = '', 'Unassigned', provider_endpoint_name)") || !strings.Contains(query, "countIf(cache_status = 'hit')") || !strings.Contains(query, "ARRAY JOIN if(empty(tags), ['Untagged'], tags)") || !strings.Contains(query, "'key' AS kind") || !strings.Contains(query, "'user' AS kind") || !strings.Contains(query, "'team' AS kind") || !strings.Contains(query, "'organization' AS kind") || !strings.Contains(query, "if(api_key_fingerprint = '', 'Unassigned', api_key_fingerprint)") || !strings.Contains(query, "sum(usage.cost) AS cost") || strings.Contains(query, "sum(cost) / count()") || strings.Contains(query, "token_hash") {
	...
	if err != nil || len(report.Totals) != 1 || report.Totals[0].TotalTokens != 15 || len(report.ByModel) != 1 || len(report.ByUpstreamModel) != 1 || report.ByUpstreamModel[0].Name != "gpt-5.6-2026-07-09" || len(report.ByEndpoint) != 1 || report.ByEndpoint[0].Name != "azure-primary" || len(report.ByTag) != 1 || report.ByTag[0].Name != "production" || len(report.ByKey) != 1 || report.ByKey[0].Name != "key-1" || len(report.ByUser) != 1 || len(report.ByTeam) != 1 || len(report.ByOrganization) != 1 {
