...
func TestClickHouseUsageReporterBuildsBoundedReport(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.Contains(query, "sum(training_tokens) AS training_tokens") || !strings.Contains(query, "sum(video_seconds) AS video_seconds") || !strings.Contains(query, "{from:UInt64}") || !strings.Contains(query, "{to:UInt64}") || r.URL.Query().Get("param_from") != "1785067200" || r.URL.Query().Get("param_to") != "1787659200" || !strings.Contains(query, "safe_db.safe_events AS usage") || !strings.Contains(query, "provider_id != ''") || !strings.Contains(query, "substring(pricing_key, position(pricing_key, '/') + 1)") || !strings.Contains(query, "startsWith(model, concat(provider_endpoint_name, '-'))") || !strings.Contains(query, "'upstream_model' AS kind") || !strings.Contains(query, "if(upstream_model = '', 'Unassigned', upstream_model)") || !strings.Contains(query, "'endpoint' AS kind") || !strings.Contains(query, "if(provider_endpoint_name = '', 'Unassigned', provider_endpoint_name)") || !strings.Contains(query, "sum(cache_read_input_tokens) AS cache_read_input_tokens") || !strings.Contains(query, "sum(cache_write_input_tokens) AS cache_write_input_tokens") || !strings.Contains(query, "sum(search_requests) AS search_requests") || !strings.Contains(query, "sum(input_characters) AS input_characters") || !strings.Contains(query, "sum(input_pages) AS input_pages") || !strings.Contains(query, "sum(input_audio_milliseconds) AS input_audio_milliseconds") || !strings.Contains(query, "sum(tool_requests) AS tool_requests") || !strings.Contains(query, "countIf(cache_status = 'hit')") || !strings.Contains(query, "ARRAY JOIN if(empty(tags), ['Untagged'], tags)") || !strings.Contains(query, "'key' AS kind") || !strings.Contains(query, "'user' AS kind") || !strings.Contains(query, "'team' AS kind") || !strings.Contains(query, "'organization' AS kind") || !strings.Contains(query, "if(api_key_fingerprint = '', 'Unassigned', api_key_fingerprint)") || !strings.Contains(query, "sum(usage.cost) AS cost") || strings.Contains(query, "sum(cost) / count()") || strings.Contains(query, "token_hash") {
