...
func TestClickHouseUsageReporterBuildsBoundedReport(t *testing.T) {
	...
	if err != nil || len(report.Totals) != 1 || report.Totals[0].TotalTokens != 15 || report.Totals[0].InputCharacters != 4096 || report.Totals[0].InputPages != 4 || report.Totals[0].InputAudioMilliseconds != 90000 || report.Totals[0].CacheReadInputTokens != 6 || report.Totals[0].CacheWriteInputTokens != 2 || report.Totals[0].SearchRequests != 2 || len(report.ByModel) != 1 || len(report.ByUpstreamModel) != 1 || report.ByUpstreamModel[0].Name != "gpt-5.6-2026-07-09" || len(report.ByEndpoint) != 1 || report.ByEndpoint[0].Name != "azure-primary" || len(report.ByTag) != 1 || report.ByTag[0].Name != "production" || len(report.ByKey) != 1 || report.ByKey[0].Name != "key-1" || len(report.ByUser) != 1 || len(report.ByTeam) != 1 || len(report.ByOrganization) != 1 {
