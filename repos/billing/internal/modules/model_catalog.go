...
func ParseModelCatalog(raw string) (ModelCatalog, error) {
	...
	for index, entry := range catalog.Models {
		...
		if entry.InputCostPer1M < 0 || entry.OutputCostPer1M < 0 {
		...
		for _, candidateModel := range models {
			if entry, found := c.entries[modelCatalogKey(candidateProvider, candidateModel)]; found {
				...
				return PricingSnapshot{
					...
					InputCostPer1M: entry.InputCostPer1M, OutputCostPer1M: entry.OutputCostPer1M, Currency: currency,
				}, nil
			}
		}
	}
...
func pricingCost(inputTokens, outputTokens int, pricing PricingSnapshot) float64 {
	return (float64(inputTokens)/1_000_000)*pricing.InputCostPer1M + (float64(outputTokens)/1_000_000)*pricing.OutputCostPer1M
}
