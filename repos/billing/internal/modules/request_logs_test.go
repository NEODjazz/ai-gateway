...
func TestClickHouseRequestLogsUseParametersAndBoundPagination(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(query, "req' OR 1=1") || strings.Contains(query, "production' OR 1=1") || !strings.Contains(query, "request_id = {request_id:String}") || !strings.Contains(query, "trace_id = {trace_id:String}") || !strings.Contains(query, "has(tags, {tag:String})") || !strings.Contains(query, "LIMIT 3") || !strings.Contains(query, "first_token_latency_ms") || !strings.Contains(query, "usage_estimated") {
	...
	page, err := reporter.ListRequestLogs(context.Background(), RequestLogFilter{Days: 7, Limit: 2, RequestID: "req' OR 1=1", TraceID: "0123456789abcdef0123456789abcdef", Tag: "production' OR 1=1", Before: time.Date(2026, 8, 26, 0, 0, 0, 0, time.UTC), BeforeRequestID: "cursor-request"})
