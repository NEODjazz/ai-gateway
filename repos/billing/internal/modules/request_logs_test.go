...
func TestClickHouseRequestLogsUseParametersAndBoundPagination(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(
			`{"timestamp":"2026-08-25T12:00:00Z","request_id":"r3","trace_id":"0123456789abcdef0123456789abcdef","organization_id":"org-1","tags":["production"],"status":"ok","api_type":"chat_completions","phase":"commit","latency_ms":40,"first_token_latency_ms":12,"retry_count":2,"fallback_count":1,"cache_status":"miss","cache_kind":"exact","input_tokens":1,"output_tokens":2,"training_tokens":1000,"total_tokens":1003,"input_characters":4096,"input_pages":4,"input_audio_milliseconds":90000,"tool_requests":3,"cache_read_input_tokens":2,"cache_write_input_tokens":1,"search_requests":2,"search_requests_estimated":false,"usage_estimated":true,"cost":0.1,"currency":"USD","content_stored":false}` + "\n" +
	...
	if err != nil || len(page.Data) != 2 || page.NextBefore != "2026-08-25T11:59:00Z" || page.NextRequestID != "r2" || page.Data[1].FailureClass != "upstream" || page.Data[0].ContentStored || page.Data[0].TraceID != "0123456789abcdef0123456789abcdef" || len(page.Data[0].Tags) != 1 || page.Data[0].Tags[0] != "production" || page.Data[0].FirstTokenLatencyMS != 12 || page.Data[0].RetryCount != 2 || page.Data[0].FallbackCount != 1 || page.Data[0].TrainingTokens != 1000 || page.Data[0].TotalTokens != 1003 || page.Data[0].InputCharacters != 4096 || page.Data[0].InputPages != 4 || page.Data[0].InputAudioMilliseconds != 90000 || page.Data[0].ToolRequests != 3 || page.Data[0].CacheReadInputTokens != 2 || page.Data[0].CacheWriteInputTokens != 1 || page.Data[0].SearchRequests != 2 || page.Data[0].SearchRequestsEstimated || !page.Data[0].UsageEstimated || page.Data[0].OrganizationID != "org-1" {
...
func TestClickHouseRequestLogGroupsAreServerAggregatedAndCursorPaginated(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		query := r.URL.Query().Get("query")
		for _, expected := range []string{"if(session_id = ''", "countIf(status = 'error')", "groupUniqArray(", "sum(training_tokens)", "sum(cache_read_input_tokens)", "sum(cache_write_input_tokens)", "sum(search_requests)", "sum(input_characters)", "sum(input_pages)", "sum(input_audio_milliseconds)", "sum(video_seconds)", "sum(output_images)", "sum(tool_requests)", "countIf(cache_status = 'hit')", "sum(usage.cost)", "usage.cost >= {min_cost:Float64}", "usage.cost <= {max_cost:Float64}", "GROUP BY group_id,currency", "before_group_id:String", "LIMIT 3"} {
		...
		_, _ = w.Write([]byte(
			`{"group_id":"session-2","requests":3,"errors":1,"models":["gpt"],"providers":["azure"],"total_tokens":1042,"training_tokens":1000,"input_characters":4096,"input_pages":4,"input_audio_milliseconds":90000,"tool_requests":3,"cache_read_input_tokens":12,"cache_write_input_tokens":4,"search_requests":2,"cache_hits":2,"latency_ms":12.5,"cost":0.3,"currency":"USD","started_at":"2026-08-25T10:00:00Z","ended_at":"2026-08-25T12:00:00Z"}` + "\n" +
	...
	if err != nil || len(page.Data) != 2 || page.Data[0].Requests != 3 || page.Data[0].TrainingTokens != 1000 || page.Data[0].TotalTokens != 1042 || page.Data[0].InputCharacters != 4096 || page.Data[0].InputPages != 4 || page.Data[0].InputAudioMilliseconds != 90000 || page.Data[0].ToolRequests != 3 || page.Data[0].CacheReadInputTokens != 12 || page.Data[0].CacheWriteInputTokens != 4 || page.Data[0].SearchRequests != 2 || page.Data[0].CacheHits != 2 || page.NextBefore != "2026-08-25T11:00:00Z" || page.NextBeforeGroupID != "session-1" || page.NextBeforeCurrency != "USD" {
		t.Fatalf("page=%+v err=%v", page, err)
	}
}
