...
func usageReportQuery(table string, scopeType string, filterModel, filterUpstreamModel, filterProvider, filterEndpoint, filterTag bool) string {
	...
	metrics := "count() AS requests, countIf(status != 'ok') AS errors, sum(input_tokens) AS input_tokens, sum(output_tokens) AS output_tokens, sum(total_tokens) AS total_tokens, sum(input_characters) AS input_characters, sum(input_pages) AS input_pages, sum(cache_read_input_tokens) AS cache_read_input_tokens, sum(cache_write_input_tokens) AS cache_write_input_tokens, sum(search_requests) AS search_requests, sum(usage.cost) AS cost, avg(latency_ms) AS avg_latency_ms, countIf(cache_status = 'hit') AS cache_hits, if(count() = 0, 0, sum(usage.cost) / count()) AS cost_per_request"
	return fmt.Sprintf(`
