...
type ResolutionAttachment = { id: string; policy_name: string; scope: string; matched_via: string[]; policy_status: "enabled" | "disabled" | "missing" | "unavailable"; dlp: boolean; av: boolean };
type ResolutionIssue = { attachment_id: string; policy_name: string; code: string };
type Resolution = { matched_attachments: ResolutionAttachment[]; effective_policies: string[]; dlp: boolean; av: boolean; enforceable: boolean; issues: ResolutionIssue[] };
...
function PolicySimulator({ client, teamOptions, keyOptions, modelOptions, tagOptions }: { client: APIClient; teamOptions: ChipOption[]; keyOptions: ChipOption[]; modelOptions: ChipOption[]; tagOptions: ChipOption[] }) {
  ...
  return <section className="policy-simulator"><div className="notice-card policy-simulator-boundary"><h2>Runtime policy simulator</h2><p>Resolve attachments using production matcher semantics. This performs no provider request, model call, DLP scan or antivirus scan.</p></div><form className="table-card policy-simulator-form" onSubmit={simulate}><div className="form-grid"><label>Team ID<input list="policy-team-options" aria-label="Team ID" value={context.team_id} onChange={(event) => setContext((current) => ({ ...current, team_id: event.target.value }))} placeholder="Select or enter team ID" /></label><label>Virtual key alias<input list="policy-key-options" aria-label="Virtual key alias" value={context.credential_alias} onChange={(event) => setContext((current) => ({ ...current, credential_alias: event.target.value }))} placeholder="Select or enter key alias" /></label><label>Virtual key ID<input aria-label="Virtual key ID" value={context.credential_id} onChange={(event) => setContext((current) => ({ ...current, credential_id: event.target.value }))} placeholder="Optional immutable ID" /></label><label>Model<input list="policy-model-options" aria-label="Model" value={context.model} onChange={(event) => setContext((current) => ({ ...current, model: event.target.value }))} placeholder="Select or enter public model" /></label></div><datalist id="policy-team-options">{teamOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</datalist><datalist id="policy-key-options">{keyOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</datalist><datalist id="policy-model-options">{modelOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</datalist><ChipMultiSelect label="Tags" options={tagOptions} value={context.tags} onChange={(tags) => setContext((current) => ({ ...current, tags }))} allowCustom />{error && <p className="form-error" role="alert">{error}</p>}<div className="modal-actions"><button type="button" className="secondary" onClick={clear}>Reset</button><button disabled={busy}>{busy ? "Simulating…" : "Simulate"}</button></div></form>
    ...
    {result && <section className="policy-resolution" aria-label="Policy resolution result"><div className="usage-stats-grid"><StatCard label="Decision" value={result.enforceable ? "Enforceable" : "Fail closed"} /><StatCard label="Matched attachments" value={result.matched_attachments.length.toLocaleString()} /><StatCard label="Effective policies" value={result.effective_policies.length.toLocaleString()} /><StatCard label="Modules" value={[result.dlp && "DLP", result.av && "AV"].filter(Boolean).join(" + ") || "None"} /></div>{result.issues.length > 0 && <section className="notice-card policy-resolution-issues"><h3>Fail-closed issues</h3>{result.issues.map((issue) => <p key={`${issue.attachment_id}-${issue.code}`}><strong>{issue.attachment_id}</strong>: {issue.policy_name} — {issue.code}</p>)}</section>}<div className="table-card"><div className="table-scroll"><table><thead><tr><th>Attachment</th><th>Policy</th><th>Matched via</th><th>Policy state</th><th>Modules</th></tr></thead><tbody>{result.matched_attachments.map((attachment) => <tr key={attachment.id}><td>{attachment.id}</td><td>{attachment.policy_name}</td><td><div className="tag-list">{attachment.matched_via.map((item) => <span className="tag" key={item}>{item}</span>)}</div></td><td><span className={`status ${attachment.policy_status === "enabled" ? "enabled" : "error"}`}>{attachment.policy_status}</span></td><td>{[attachment.dlp && "DLP", attachment.av && "AV"].filter(Boolean).join(" + ") || "—"}</td></tr>)}</tbody></table>{!result.matched_attachments.length && <div className="empty-state"><strong>No attachments matched</strong></div>}</div></div></section>}
  </section>;
}
