...
export function VirtualKeyDetailsPage() {
  ...
  return <><PageHeader eyebrow="Access control" title={key.alias || key.id} description="Virtual-key ownership, effective grants, usage, budgets and lifecycle." actions={<div className="inline-actions"><button className="secondary" onClick={() => navigate("/api-keys")}>Back to virtual keys</button><ActionsMenu label={`Actions for ${key.alias || key.id}`} items={actionItems} /></div>} />
    ...
    <PageTabs label="Virtual key details" value={tab} items={[{ value: "overview", label: "Overview" }, { value: "usage", label: "Usage & budgets" }, { value: "policies", label: "Policies" }, { value: "settings", label: "Settings" }]} onUpdate={setTab} />
    ...
    {issued && <ModalFrame label="Virtual key rotated" onClose={() => setIssued(undefined)}><section className="modal issued-key-modal"><div className="modal-heading"><h2>Save this rotated key now</h2><button className="icon-button" aria-label="Close issued key" onClick={() => setIssued(undefined)}>×</button></div><p>This plaintext token is shown exactly once. It cannot be recovered after this window is closed.</p><label>Virtual key<input readOnly value={issued.token} onFocus={(event) => event.currentTarget.select()} /></label>{copyError && <p className="form-error" role="alert">{copyError}</p>}{copied && <div className="copy-confirmation" role="status"><span aria-hidden="true">✓</span> Copied to clipboard</div>}<div className="modal-actions"><button className="secondary" onClick={() => setIssued(undefined)}>Close</button><button onClick={() => void copyToken()}>{copied ? "Copy again" : "Copy"}</button></div></section></ModalFrame>}
  </>;
}
