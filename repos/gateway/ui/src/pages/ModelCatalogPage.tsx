...
export function ModelCatalogPage() {
  ...
  return <><PageHeader eyebrow="Catalog" title="Models" description="Canonical provider/model pricing and capability metadata. Deployments remain separate and cannot create duplicate catalog identities." actions={<><label className="button-link file-button">Import pricing<input aria-label="Pricing JSON file" type="file" accept="application/json,.json" onChange={(event) => void selectImport(event.target.files?.[0])} /></label><button className="secondary" onClick={() => void load()}>Refresh</button><button onClick={() => setEditing(null)}>Add model</button></>} />
