...
describe("RouterSettingsPage", () => {
  it("saves retry, fallback and deployment settings before simulating", async () => {
    const groups = [
      { id: "public-chat", deployment_ids: ["primary", "fallback"], strategy: "weighted", retry_policy: { timeout: 1 }, enabled: true },
      { id: "safe-chat", deployment_ids: ["safe"], strategy: "weighted", enabled: true }
    ];
    const deployments = [
      ...
      { id: "safe", provider_id: "azure", models: ["safe-chat"], upstream_model: "safe-gpt", priority: 0, weight: 1, enabled: true }
    ];
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (input, options) => {
      ...
      if (path === "/admin/v1/model-groups/public-chat/routing-settings" && options?.method === "PUT") {
        ...
        return response({ revision: 8, model_group: { ...groups[0], deployment_ids: body.deployment_ids, retry_policy: body.retry_policy, fallbacks: body.fallbacks }, deployments: body.deployments.map((row: { id: string }) => ({ ...deployments.find((item) => item.id === row.id), ...row })) });
      }
    ...
    await userEvent.selectOptions(screen.getByLabelText("general fallback model group"), "safe-chat");
    ...
    expect(routingBody).toMatchObject({ expected_revision: 7, deployment_ids: ["fallback", "primary"], retry_policy: { timeout: 1, rate_limit: 3 }, fallbacks: { general: ["safe-chat"] }, deployments: [{ id: "fallback", priority: 0 }, { id: "primary", priority: 1 }] });
    ...
    const simulationCall = fetchMock.mock.calls.find(([path]) => String(path) === "/admin/v1/routing/simulate")!;
    expect(JSON.parse(String(simulationCall[1]?.body))).toMatchObject({ model: "public-chat", failure_class: "unavailable" });
