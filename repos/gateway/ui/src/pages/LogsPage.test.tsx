...
describe("LogsPage", () => {
  ...
  it("restores a shareable request-log view and opens request details from the URL", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (path.includes("request-logs/groups")) return json({ data: [{ group_id: "session-1", requests: 2, errors: 1, models: ["gpt"], providers: ["azure"], total_tokens: 20, cache_read_input_tokens: 8, cache_write_input_tokens: 2, cache_hits: 1, latency_ms: 100, cost: 0.2, currency: "USD", started_at: "2026-08-28T10:00:00Z", ended_at: "2026-08-28T11:00:00Z" }] });
      return json({ data: [] });
    });
