...
export function UsagePage() {
  ...
  function exportCSV() {
    ...
    for (const [type, rows] of [["public_model", report.by_model || []], ["upstream_model", report.by_upstream_model || []], ["provider", report.by_provider || []], ["endpoint", report.by_endpoint || []], ["tag", report.by_tag || []], ["key", report.by_key || []], ["user", report.by_user || []], ["team", report.by_team || []], ["organization", report.by_organization || []]] as const) for (const row of rows) lines.push([JSON.stringify(row.name || ""), type, row.requests, row.errors, row.input_tokens, row.output_tokens, row.total_tokens, row.cache_read_input_tokens || 0, row.cache_write_input_tokens || 0, row.cache_hits, row.avg_latency_ms, row.cost, JSON.stringify(row.currency)].join(","));
  ...
  return <><PageHeader eyebrow="Analytics" title="Usage & spend" description="Gateway activity, spend, token and reliability trends without combining currencies." actions={<><label className="usage-window"><span className="sr-only">Period</span><select aria-label="Window" form="usage-filters" value={draft.window} onChange={(event) => setDraft({ ...draft, window: event.target.value })}><option value="7">7 days</option><option value="30">30 days</option><option value="90">90 days</option><option value="custom">Custom range</option></select></label><GatewayButton view="outlined" size="l" disabled={!report} onClick={exportCSV}>Export CSV</GatewayButton></>} />
