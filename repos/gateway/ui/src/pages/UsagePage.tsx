...
type UsageReport = { days?: number; from?: string; to?: string; totals: UsageAggregate[]; daily: UsageAggregate[]; by_model: UsageAggregate[]; by_provider: UsageAggregate[]; by_tag: UsageAggregate[] };
type UsageFilters = { window: string; from: string; to: string; model: string; provider: string; tag: string };
type UsageView = "overview" | "models" | "providers" | "tags";
...
export function UsagePage() {
  ...
  function exportCSV() {
    ...
    for (const [type, rows] of [["model", report.by_model || []], ["provider", report.by_provider || []], ["tag", report.by_tag || []]] as const) for (const row of rows) lines.push([JSON.stringify(row.name || ""), type, row.requests, row.errors, row.input_tokens, row.output_tokens, row.total_tokens, row.cache_hits, row.avg_latency_ms, row.cost, JSON.stringify(row.currency)].join(","));
