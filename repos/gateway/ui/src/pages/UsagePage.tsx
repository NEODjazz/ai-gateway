...
type UsageReport = {
  ...
  by_model: UsageAggregate[]; by_provider: UsageAggregate[]; by_tag: UsageAggregate[]; by_key: UsageAggregate[];
...
type UsageView = "overview" | "models" | "providers" | "tags" | "keys" | "users" | "teams" | "organizations";
...
const usageViews: { id: UsageView; label: string }[] = [
  { id: "overview", label: "Overview" }, { id: "models", label: "Models" }, { id: "providers", label: "Providers" }, { id: "tags", label: "Tags" },
...
export function UsagePage() {
  ...
  function exportCSV() {
    ...
    for (const [type, rows] of [["model", report.by_model || []], ["provider", report.by_provider || []], ["tag", report.by_tag || []], ["key", report.by_key || []], ["user", report.by_user || []], ["team", report.by_team || []], ["organization", report.by_organization || []]] as const) for (const row of rows) lines.push([JSON.stringify(row.name || ""), type, row.requests, row.errors, row.input_tokens, row.output_tokens, row.total_tokens, row.cache_hits, row.avg_latency_ms, row.cost, JSON.stringify(row.currency)].join(","));
