...
type UsageAggregate = {
  ...
  training_tokens: number; total_tokens: number; input_characters: number; input_pages: number; input_audio_milliseconds: number; tool_requests: number; cache_read_input_tokens: number; cache_write_input_tokens: number; search_requests: number; cost: number; avg_latency_ms: number; cache_hits: number; cost_per_request: number;
};
type UsageReport = {
...
function aggregateDailyActivity(rows: UsageAggregate[]) {
  ...
  for (const row of rows) {
    const date = row.date || "Unknown"; const current = grouped.get(date) || { date, currency: "", requests: 0, errors: 0, input_tokens: 0, output_tokens: 0, training_tokens: 0, total_tokens: 0, input_characters: 0, input_pages: 0, input_audio_milliseconds: 0, tool_requests: 0, cache_read_input_tokens: 0, cache_write_input_tokens: 0, search_requests: 0, cost: 0, avg_latency_ms: 0, cache_hits: 0, cost_per_request: 0 };
...
export function UsagePage() {
  ...
  function exportCSV() {
    if (!report) return;
    const lines = ["dimension,type,requests,errors,input_tokens,output_tokens,training_tokens,total_tokens,input_characters,input_pages,input_audio_milliseconds,tool_requests,cache_read_input_tokens,cache_write_input_tokens,search_requests,cache_hits,avg_latency_ms,cost,currency"];
