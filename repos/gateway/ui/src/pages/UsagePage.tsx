...
type UsageAggregate = {
  ...
  total_tokens: number; input_characters: number; cache_read_input_tokens: number; cache_write_input_tokens: number; search_requests: number; cost: number; avg_latency_ms: number; cache_hits: number; cost_per_request: number;
};
type UsageReport = {
...
function aggregateDailyActivity(rows: UsageAggregate[]) {
  ...
  for (const row of rows) {
    const date = row.date || "Unknown"; const current = grouped.get(date) || { date, currency: "", requests: 0, errors: 0, input_tokens: 0, output_tokens: 0, total_tokens: 0, input_characters: 0, cache_read_input_tokens: 0, cache_write_input_tokens: 0, search_requests: 0, cost: 0, avg_latency_ms: 0, cache_hits: 0, cost_per_request: 0 };
    const latencyTotal = current.avg_latency_ms * current.requests + row.avg_latency_ms * row.requests;
    current.requests += row.requests; current.errors += row.errors; current.input_tokens += row.input_tokens; current.output_tokens += row.output_tokens; current.total_tokens += row.total_tokens; current.input_characters += row.input_characters || 0; current.cache_read_input_tokens += row.cache_read_input_tokens || 0; current.cache_write_input_tokens += row.cache_write_input_tokens || 0; current.search_requests += row.search_requests || 0; current.cache_hits += row.cache_hits;
...
const usageColumns = [
  ...
  { key: "output_tokens", label: "Output tokens" }, { key: "input_characters", label: "Input characters" }, { key: "cache_read_input_tokens", label: "Cache read tokens" }, { key: "cache_write_input_tokens", label: "Cache write tokens" }, { key: "search_requests", label: "Searches" }, { key: "cache_hits", label: "Cache hits" }, { key: "latency", label: "Average latency" },
...
export function UsagePage() {
  ...
  function exportCSV() {
    if (!report) return;
    const lines = ["dimension,type,requests,errors,input_tokens,output_tokens,total_tokens,input_characters,cache_read_input_tokens,cache_write_input_tokens,search_requests,cache_hits,avg_latency_ms,cost,currency"];
    for (const [type, rows] of [["public_model", report.by_model || []], ["upstream_model", report.by_upstream_model || []], ["provider", report.by_provider || []], ["endpoint", report.by_endpoint || []], ["tag", report.by_tag || []], ["key", report.by_key || []], ["user", report.by_user || []], ["team", report.by_team || []], ["organization", report.by_organization || []]] as const) for (const row of rows) lines.push([row.name || "", type, row.requests, row.errors, row.input_tokens, row.output_tokens, row.total_tokens, row.input_characters || 0, row.cache_read_input_tokens || 0, row.cache_write_input_tokens || 0, row.search_requests || 0, row.cache_hits, row.avg_latency_ms, row.cost, row.currency].map(csvCell).join(","));
