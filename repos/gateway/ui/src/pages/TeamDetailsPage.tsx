...
function suggestedAttachmentID(identity: string) {
...
function MembershipForm({ initial, users, onClose, onSave }: { initial?: Membership; users: User[]; onClose: () => void; onSave: (userID: string, roles: string[]) => Promise<void> }) {
  ...
  return <ModalFrame label={initial ? "Edit team member" : "Add team member"} onClose={onClose}><section className="modal compact-modal"><div className="modal-heading"><h2>{initial ? "Edit team member" : "Add team member"}</h2><button className="icon-button" aria-label="Close member form" onClick={onClose}>×</button></div><form onSubmit={submit}><div className="form-grid"><label className="span-2">User<select aria-label="Team member user" required disabled={Boolean(initial)} value={userID} onChange={(event) => setUserID(event.target.value)}><option value="">Select configured user</option>{users.map((user) => <option key={user.id} value={user.id}>{user.name || user.email || user.id} · {user.id}</option>)}</select></label><div className="span-2"><ChipMultiSelect label="Team roles" options={[{ value: "member", label: "Member" }, { value: "developer", label: "Developer" }, { value: "team_admin", label: "Team admin" }]} value={roles} onChange={setRoles} allowCustom /></div></div>{error && <p className="form-error" role="alert">{error}</p>}<div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancel</button><button disabled={saving || !userID}>{saving ? "Saving…" : "Save membership"}</button></div></form></section></ModalFrame>;
}
...
export function TeamDetailsPage() {
  ...
  const policyActions = [{ label: "Create team attachment", onSelect: () => { const query = new URLSearchParams({ create: "1", attach_team: team.id }); const suggestion = suggestedAttachmentID(team.id); if (suggestion) query.set("suggested_id", suggestion); navigate(`/policies?${query.toString()}`); } }, { label: "Simulate policy impact", onSelect: () => navigate(`/policies?view=simulator&team_id=${encodeURIComponent(team.id)}`) }];
  return <><PageHeader eyebrow="Access control" title={team.name} description="Membership roles and team-owned virtual keys in one scoped workspace." actions={<div className="inline-actions"><button className="secondary" onClick={() => navigate("/teams")}>Back to teams</button>{isAdmin && <ActionsMenu label={`Policy actions for ${team.name}`} items={policyActions} />}</div>} />{error && <ErrorState message={error} retry={() => void load()} />}<div className="usage-stats-grid"><StatCard label="Members" value={memberships.length.toLocaleString()} /><StatCard label="Virtual keys" value={isAdmin ? keys.length.toLocaleString() : "Scoped"} detail={isAdmin ? "first 100 matching keys" : "admin-only inventory"} /><StatCard label="Status" value={team.status} /><StatCard label="Updated" value={formatTimestamp(team.updated_at)} /></div><section className="table-card access-group-details"><h2>Team details</h2><dl className="detail-grid"><div><dt>Team ID</dt><dd>{team.id}</dd></div><div><dt>Created</dt><dd>{formatTimestamp(team.created_at)}</dd></div><div><dt>Description</dt><dd>{team.description || "—"}</dd></div><div><dt>Access boundary</dt><dd>{isAdmin ? "Global administrator" : "Matching team administrator"}</dd></div></dl>{isAdmin && <p className="muted">Policy impact also depends on virtual key, credential tags and requested model. Use policy actions to create a team-scoped attachment or simulate a complete request context.</p>}</section><section className="section-block"><h2>Members</h2><p>Roles are attached to this team membership, independently from directory-wide user roles.</p><ManagedDataTable rows={memberRows} columns={[{ key: "user", label: "User" }, { key: "email", label: "Email" }, { key: "roles", label: "Team roles" }, { key: "status", label: "Status" }, { key: "joined", label: "Joined" }]} primaryAction={<button disabled={!availableUsers.length} onClick={() => setEditing(null)}>Add member</button>} onRefresh={load} searchPlaceholder="Search team members" actions={(row) => <ActionsMenu label={`Actions for member ${row.id}`} items={[{ label: "Edit roles", onSelect: () => setEditing(row._membership as Membership) }, { label: "Remove", tone: "danger", onSelect: () => void removeMembership(String(row.id)) }]} />} /></section><section className="section-block"><h2>Virtual keys</h2><p>{isAdmin ? "Keys directly owned by this team or inherited through membership-aware ownership." : "Virtual-key inventory is restricted to global administrators."}</p>{isAdmin ? <ManagedDataTable rows={keyRows} columns={[{ key: "id", label: "Key" }, { key: "alias", label: "Alias" }, { key: "status", label: "Status" }, { key: "allowed_models", label: "Models" }, { key: "budget", label: "Budget" }]} onRefresh={load} searchPlaceholder="Search team keys" actions={(row) => <ActionsMenu label={`Actions for virtual key ${row.id}`} items={[{ label: "Inspect", href: `/api-keys/${encodeURIComponent(String(row.id))}` }]} />} /> : <div className="empty-state">No global key inventory in this scoped session.</div>}</section>{editing !== undefined && <MembershipForm initial={editing || undefined} users={editing ? users : availableUsers} onClose={() => setEditing(undefined)} onSave={saveMembership} />}</>;
}
