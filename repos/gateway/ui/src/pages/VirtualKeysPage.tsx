...
const keyColumns: ColumnChoice[] = [
  ...
  { key: "spend", label: "Spend (30d)" }, { key: "requests", label: "Requests (30d)" }, { key: "tokens", label: "Tokens (30d)" }, { key: "budget", label: "Budget" },
  { key: "status", label: "Status" }, { key: "models", label: "Models" }, { key: "description", label: "Description" },
...
const defaultKeyColumns = new Set(["key", "alias", "organization", "team", "user", "created", "spend", "budget", "status", "models"]);
...
export function VirtualKeysPage() {
  ...
  const [keys, setKeys] = useState<VirtualKey[]>([]); const [users, setUsers] = useState<User[]>([]); const [teams, setTeams] = useState<Team[]>([]); const [organizations, setOrganizations] = useState<Organization[]>([]); const [models, setModels] = useState<string[]>([]); const [budgets, setBudgets] = useState<Budget[]>([]); const [usage, setUsage] = useState<KeyUsage[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [userPayload, teamPayload, organizationPayload, modelPayload, budgetPayload, usagePayload] = await Promise.all([safe("/admin/v1/users?limit=500"), safe("/admin/v1/teams?limit=500"), safe("/admin/v1/organizations?limit=500"), safe("/v1/models"), safe("/admin/v1/budgets"), safe("/admin/v1/usage/report?days=30")]);
      setKeys(records<VirtualKey>(keyPayload)); setUsers(records<User>(userPayload)); setTeams(records<Team>(teamPayload)); setOrganizations(records<Organization>(organizationPayload)); setBudgets(records<Budget>(budgetPayload)); setUsage(records<KeyUsage>((usagePayload as { by_key?: unknown }).by_key));
