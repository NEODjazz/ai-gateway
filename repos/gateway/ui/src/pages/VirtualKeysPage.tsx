...
export function VirtualKeysPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState(() => new Set(defaultKeyColumns));
	...
	const loadReferences = useCallback(async () => {
		const [userPayload, teamPayload, organizationPayload, modelPayload, usagePayload] = await Promise.all([safe("/admin/v1/users?limit=500"), safe("/admin/v1/teams?limit=500"), safe("/admin/v1/organizations?limit=500"), safe("/v1/models"), safe("/admin/v1/usage/report?days=30")]);
		setUsers(records<User>(userPayload)); setTeams(records<Team>(teamPayload)); setOrganizations(records<Organization>(organizationPayload)); setUsage(records<KeyUsage>((usagePayload as { by_key?: unknown }).by_key));
