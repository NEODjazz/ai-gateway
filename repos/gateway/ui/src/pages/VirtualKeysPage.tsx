...
export function VirtualKeysPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState(() => new Set(defaultKeyColumns));
	...
	const loadReferences = useCallback(async () => {
		...
		setUsers(records<User>(userPayload)); setTeams(records<Team>(teamPayload)); setOrganizations(records<Organization>(organizationPayload)); setBudgets(records<Budget>(budgetPayload)); setUsage(records<KeyUsage>((usagePayload as { by_key?: unknown }).by_key));
		setModels([...new Set(records<Model>(modelPayload).map((model) => model.id || model.model || "").filter(Boolean))].sort());
	}, [safe]);
	const loadKeys = useCallback(async () => {
		setLoading(true); setError("");
		try {
			const query = new URLSearchParams({ limit: String(pageSize), offset: String(page * pageSize), sort_by: sort === "budget" ? "created" : sort, sort_order: direction });
			...
			const payload = await client.request<{ data?: VirtualKey[]; total?: number }>(`/admin/v1/keys?${query.toString()}`);
			setKeys(records<VirtualKey>(payload)); setTotal(Number(payload.total || 0));
	...
	const rows = useMemo(() => sort !== "budget" ? keys : [...keys].sort((a, b) => { const left = budgetValue(budgetFor(a, budgets)); const right = budgetValue(budgetFor(b, budgets)); const compared = left === right ? 0 : left < right ? -1 : 1; return direction === "asc" ? compared : -compared; }), [budgets, direction, keys, sort]);
