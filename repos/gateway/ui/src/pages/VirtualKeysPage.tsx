...
function KeyForm({ initial, users, teams, organizations, accessGroups, models, onClose, onSubmit }: { initial?: VirtualKey; users: User[]; teams: Team[]; organizations: Organization[]; accessGroups: AccessGroup[]; models: string[]; onClose: () => void; onSubmit: (value: FormState) => Promise<void> }) {
  ...
  return <ModalFrame label={initial ? "Edit virtual key" : "Create Virtual Key"} onClose={onClose}><section className="modal key-form-modal"><div className="modal-heading"><h2>{initial ? "Edit virtual key" : "Create Virtual Key"}</h2><button className="icon-button" aria-label="Close" onClick={onClose}>×</button></div><form onSubmit={submit} className="key-form">
...
export function VirtualKeysPage() {
  ...
  return <><PageHeader eyebrow="Access control" title="Virtual keys" description="Issue scoped, revocable gateway credentials. Plaintext tokens are displayed exactly once." />
    <div className="key-toolbar"><GatewayButton size="l" onClick={() => setForm("create")}>Create Virtual Key</GatewayButton><div className="key-toolbar-right"><GravityThemeScope className="gravity-search-scope"><TextInput className="key-search" size="l" type="search" controlProps={{ "aria-label": "Search keys by alias" }} placeholder="Search by alias" value={search} startContent={<Icon data={Magnifier} size={16} />} onUpdate={(value) => { setPage(0); setSearch(value); }} /></GravityThemeScope><ColumnsMenu columns={keyColumns} visible={visibleColumns} onChange={setVisibleColumns} /><ToolbarIconButton icon="filter" label="Filter" active={activeFilters > 0} onClick={() => setFilterOpen(true)} /><ToolbarIconButton icon="refresh" label="Refresh virtual keys" onClick={() => void load()} /></div></div>
    ...
    {issued && <ModalFrame label="Virtual key created" onClose={() => setIssued(undefined)}><section className="modal issued-key-modal"><div className="modal-heading"><h2>Save this key now</h2><button className="icon-button" aria-label="Close issued key" onClick={() => setIssued(undefined)}>×</button></div><p>This plaintext token is shown exactly once. It cannot be recovered after this window is closed.</p><label>Virtual key<input readOnly value={issued.token} onFocus={(event) => event.currentTarget.select()} /></label>{copyError && <p className="form-error" role="alert">{copyError}</p>}{copied && <div className="copy-confirmation" role="status"><span aria-hidden="true">✓</span> Copied to clipboard</div>}<div className="modal-actions"><button className="secondary" onClick={() => setIssued(undefined)}>Close</button><button onClick={() => void copyToken()}>{copied ? "Copy again" : "Copy"}</button></div></section></ModalFrame>}
  </>;
}
