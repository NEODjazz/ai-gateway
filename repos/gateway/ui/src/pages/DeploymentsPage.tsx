...
export function DeploymentsPage() {
  ...
  const tableRows = useMemo(() => deployments.map((row) => ({ ...row, selected: selected.has(row.id), last_health: latest[row.id]?.status, health_latency_ms: latest[row.id]?.latency_ms })), [deployments, latest, selected]);
  const tableColumns = useMemo(() => [
    ...
    { key: "selected", label: "Selected", render: (_: unknown, value: Row) => <input aria-label={`Select ${String(value.id)}`} type="checkbox" checked={selected.has(String(value.id))} onChange={(event) => setSelected((current) => { const next = new Set(current); if (event.target.checked) next.add(String(value.id)); else next.delete(String(value.id)); return next; })} /> }
  ], [selected]);
  ...
  return <><PageHeader eyebrow="Runtime routing" title="Deployments" description="Provider/model endpoints with live health, circuit state and operational controls." />
    ...
    {loading ? <LoadingState /> : <ManagedDataTable rows={tableRows} columns={tableColumns} rowKey="id" defaultHidden={["priority", "weight", "selected"]} primaryAction={<div className="inline-actions"><Link className="button-link" to="/model-onboarding">Onboard models</Link><label className="checkbox-line compact-check"><input aria-label="Select all deployments" type="checkbox" checked={allSelected} onChange={(event) => setSelected(event.target.checked ? new Set(deployments.map((row) => row.id)) : new Set())} /> Select page</label><button disabled={busy || !deployments.length} onClick={() => void runChecks(selectedIDs.length ? selectedIDs : deployments.map((row) => row.id))}>{busy ? "Checking…" : selectedIDs.length ? `Check selected (${selectedIDs.length})` : "Check page"}</button></div>} toolbarExtra={<button className="secondary" onClick={() => setFiltersOpen(true)}>Filter{providerFilter || stateFilter ? " (active)" : ""}</button>} onRefresh={load} searchPlaceholder="Search deployments" server={{ search, onSearchChange: (value) => { setOffset(0); setSearch(value); }, total, offset, pageSize: limit, onPageSizeChange: (value) => { setOffset(0); setLimit(value); }, onOffsetChange: setOffset, sort: Object.keys(deploymentSortKeys).find((key) => deploymentSortKeys[key] === sort) || "priority", direction: order as "asc" | "desc", sortableKeys: Object.keys(deploymentSortKeys), onSortChange: (key, direction) => { setOffset(0); setSort(deploymentSortKeys[key]); setOrder(direction); } }} actions={(value) => { const row = value as Deployment; return <ActionsMenu label={`Actions for ${row.id}`} items={[{ label: "Details", onSelect: () => openDetails(row) }, { label: "Check", onSelect: () => runChecks([row.id]) }, { label: "Edit", onSelect: () => setEditing(row) }, { label: row.enabled ? "Pause" : "Resume", tone: row.enabled ? "danger" : "default", onSelect: () => toggle(row) }]} />; }} />}
    {filtersOpen && <div className="modal-backdrop" role="presentation"><form className="modal compact-modal" role="dialog" aria-modal="true" aria-label="Filter deployments" onSubmit={(event) => { event.preventDefault(); setOffset(0); setFiltersOpen(false); void load(); }}><div className="modal-heading"><h2>Filter deployments</h2><button type="button" className="icon-button" aria-label="Close filters" onClick={() => setFiltersOpen(false)}>×</button></div><div className="form-grid"><label>Provider<input value={providerFilter} onChange={(event) => setProviderFilter(event.target.value)} /></label><label>Runtime<select value={stateFilter} onChange={(event) => setStateFilter(event.target.value)}><option value="">All</option><option value="available">Available</option><option value="cooling_down">Cooling down</option><option value="disabled">Disabled</option></select></label></div><div className="modal-actions"><button type="button" className="secondary" onClick={() => { setOffset(0); setProviderFilter(""); setStateFilter(""); }}>Reset filters</button><button>Apply filters</button></div></form></div>}
