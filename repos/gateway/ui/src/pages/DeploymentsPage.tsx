...
import { providerModelCapabilityOptions } from "../modelCapabilities";
...
type ProviderCapabilityProfile = { type: string; capabilities?: string[]; operations?: string[] };
...
function deploymentPayload(row: Deployment, enabled = row.enabled) {
  return {
    ...
    queue_capacity: Number(row.queue_capacity || 0), queue_timeout_ms: Number(row.queue_timeout_ms || 0),
    rate_limit_rpm: Number(row.rate_limit_rpm || 0), rate_limit_tpm: Number(row.rate_limit_tpm || 0), enabled
  };
}
...
export function DeploymentsPage() {
  ...
  async function remove(row: Deployment) {
    if (!window.confirm(`Delete deployment ${row.id}?`)) return;
    setBusy(true); setError("");
    try {
      await client.request(`/admin/v1/model-deployments/${encodeURIComponent(row.id)}`, { method: "DELETE" });
      setSelected((current) => { const next = new Set(current); next.delete(row.id); return next; });
      if (detail?.id === row.id) setDetail(undefined);
      if (editing?.id === row.id) setEditing(undefined);
      await load();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not delete deployment"); }
    finally { setBusy(false); }
  }
  ...
  const editFields = useMemo(() => (resourceConfigs.deployments.fields || []).map((field) => field.key === "capabilities" && editing
