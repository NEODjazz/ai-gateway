...
function GroupForm({ initial, deployments, onClose, onSave }: { initial?: ModelGroup; deployments: Deployment[]; onClose: () => void; onSave: (value: GroupDraft) => Promise<void> }) {
  ...
  return <div className="modal-backdrop" role="presentation"><form className="modal model-group-form-modal" role="dialog" aria-modal="true" aria-label={`${initial ? "Edit" : "Create"} model group`} onSubmit={submit}>
    <div className="modal-heading"><div><h2>{initial ? "Edit" : "Create"} Model Group</h2><span className="muted">A public model routed across managed deployments.</span></div><button type="button" className="icon-button" aria-label="Close model group form" onClick={onClose}>×</button></div>
    ...
    {error && <p className="form-error" role="alert">{error}</p>}<div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancel</button><button disabled={busy}>{busy ? "Saving…" : initial ? "Save changes" : "Create group"}</button></div>
  </form></div>;
}
