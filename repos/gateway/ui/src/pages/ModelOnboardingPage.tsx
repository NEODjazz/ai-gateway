...
export function ModelOnboardingPage() {
  ...
  const canReview = selectedCandidates.length > 0 && selectedCandidates.every((candidate) =>
    candidate.publicModel.trim() && candidate.deploymentID.trim() && candidate.capabilities.length > 0 && hasAzurePricing(candidate));
  const canApply = canReview;
