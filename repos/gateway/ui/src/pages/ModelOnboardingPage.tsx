  ...
  async function discover(event: FormEvent) {
    ...
    try {
      ...
      const [probeResult, discovery] = await Promise.all([
        ...
        client.request<{ data?: Array<{ id: string; capabilities?: string[] }> }>(`/admin/v1/providers/${encodeURIComponent(selected.providerID)}/discover-models`, { method: "POST", body })
      ]);
      setProbe(probeResult);
      setCandidates((discovery.data || []).map(({ id, capabilities }) => ({ upstream: id, selected: false, publicModel: id, deploymentID: safeID(`${selected.providerID}-${id}`), capabilities: selected.providerType === "ollama" ? capabilities || [] : defaultModelCapabilities(selected.providerType), inputCost: "", outputCost: "", currency: "USD" })));
      setStep(2);
