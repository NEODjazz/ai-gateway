...
export function PlaygroundPage() {
  ...
  return <>
    <PageHeader eyebrow="Inference" title="Playground" description="Run Chat Completions or Responses through the authenticated gateway with model discovery, streaming and session continuity." actions={<button className="secondary" disabled={running} onClick={newSession}>New session</button>} />
    <div className="page-tabs" role="tablist" aria-label="Playground API"><button role="tab" aria-selected={mode === "chat"} className={mode === "chat" ? "active" : ""} onClick={() => changeMode("chat")}>Chat Completions</button><button role="tab" aria-selected={mode === "responses"} className={mode === "responses" ? "active" : ""} onClick={() => changeMode("responses")}>Responses API</button></div>
    <div className="playground-grid">
      <form className="form-card playground-form" onSubmit={submit}>
        <label>Model<input aria-label="Model" disabled={running} required list="playground-models" value={model} onChange={(event) => { setModel(event.target.value); setPreviousResponseID(""); }} placeholder={loadingModels ? "Loading models…" : "Select or enter a logical model"} /></label><datalist id="playground-models">{models.map((item) => <option key={item} value={item} />)}</datalist>
        <div className="playground-model-help"><span>{models.length ? `${models.length.toLocaleString()} authorized models` : "Manual model entry is available"}</span><button type="button" className="text-button" disabled={loadingModels} onClick={() => void loadModels()}>Refresh models</button></div>
        {modelsError && <p className="form-error" role="status">Model discovery: {modelsError}</p>}
        <label>Instructions<textarea aria-label="Instructions" disabled={running} rows={3} value={instructions} onChange={(event) => setInstructions(event.target.value)} placeholder={mode === "chat" ? "Optional system message" : "Optional Responses instructions"} /></label>
        <label>Message<textarea aria-label="Message" disabled={running} required rows={8} value={message} onChange={(event) => setMessage(event.target.value)} /></label>
        <div className="playground-settings"><label>Maximum output tokens<input aria-label="Maximum output tokens" disabled={running} type="number" min="1" step="1" value={maxTokens} onChange={(event) => setMaxTokens(event.target.value)} placeholder="Provider default" /></label><label>Temperature<input aria-label="Temperature" disabled={running} type="number" min="0" max="2" step="0.1" value={temperature} onChange={(event) => setTemperature(event.target.value)} placeholder="Provider default" /></label></div>
        <p className="muted playground-default-note">Leave optional settings blank to use provider defaults for models that restrict custom sampling values.</p>
        <label className="checkbox-line"><input aria-label="Stream response" disabled={running} type="checkbox" checked={streaming} onChange={(event) => setStreaming(event.target.checked)} /> Stream response</label>
        {streaming && <p className="muted playground-default-note">If the selected deployment cannot stream this API, the gateway may return one complete JSON response instead.</p>}
        {error && <p role="alert" className="form-error">{error}</p>}
        <div className="playground-actions"><button disabled={running || !model.trim() || !message.trim()}>{running ? "Running…" : transcript.length ? "Send message" : "Run request"}</button>{running && <button type="button" className="secondary" onClick={() => abortRef.current?.abort()}>Stop</button>}</div>
      </form>
      <section className="output-card playground-output" aria-label="Playground conversation">
        <div className="playground-output-heading"><div><h2>Conversation</h2><span className="muted">Session <code>{activeSessionID}</code></span></div>{transcript.length > 0 && <button className="text-button" disabled={running} onClick={newSession}>Clear</button>}</div>
        {!transcript.length && !pendingOutput && <p className="playground-empty">Run a request to start an in-memory conversation. Prompt and response content is not persisted by the console.</p>}
        <div className="playground-transcript">{transcript.map((turn) => <article className={`playground-turn ${turn.role}`} key={turn.id}><strong>{turn.role === "user" ? "You" : "Assistant"}</strong><pre>{turn.content}</pre></article>)}{pendingOutput && <article className="playground-turn assistant streaming"><strong>Assistant <span>streaming</span></strong><pre>{pendingOutput}</pre></article>}</div>
        {metadata && <dl className="playground-metadata"><div><dt>API</dt><dd>{mode === "chat" ? "Chat Completions" : "Responses"}{metadata.streamed ? " · streamed" : ""}</dd></div><div><dt>Model</dt><dd>{metadata.model || model}</dd></div><div><dt>Response ID</dt><dd>{metadata.id || "—"}</dd></div><div><dt>Tokens</dt><dd>{tokenCount ?? "—"}</dd></div></dl>}
        {events.length > 0 && <details className="playground-events"><summary>Stream events ({events.length}{eventCount > events.length ? "+" : ""})</summary><pre>{events.map((item) => `${item.event}: ${item.data}`).join("\n\n")}</pre></details>}
      </section>
    </div>
  </>;
}
