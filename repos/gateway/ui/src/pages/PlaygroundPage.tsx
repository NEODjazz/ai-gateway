...
export function PlaygroundPage() {
  ...
  return <>
    ...
    <div className="playground-selection-bar"><label>Model<div className="playground-model-control"><GravityThemeScope className="gravity-playground-control"><TextInput aria-label="Model" size="l" disabled={running} controlProps={{ required: true, list: "playground-models" }} value={model} onUpdate={(value) => { setModel(value); setPreviousResponseID(""); }} placeholder={loadingModels ? "Loading models…" : "Select or enter a logical model"} /></GravityThemeScope><ToolbarIconButton icon="refresh" label="Refresh models" disabled={loadingModels} onClick={() => void loadModels()} /></div><span className="playground-model-help">{models.length ? `${models.length.toLocaleString()} authorized models` : "Manual model entry is available"}</span></label><datalist id="playground-models">{models.map((item) => <option key={item} value={item} />)}</datalist><label>Deployment<GravityThemeScope className="gravity-playground-control"><Select aria-label="Deployment" size="l" width="max" disabled value={["automatic"]} options={[{ value: "automatic", content: "Automatic gateway routing" }]} /></GravityThemeScope><span className="playground-model-help">Resolved by routing policy</span></label><GatewayButton view="outlined" size="l" disabled={running} onClick={newSession}>Clear</GatewayButton></div>
