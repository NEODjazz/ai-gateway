...
function GuardrailPolicyForm({ initial, client, onClose, onSaved }: { initial?: Policy; client: APIClient; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <ResourceForm title={initial ? `Edit ${initial.name}` : "Create Guardrail Policy"} fields={policyFields} initial={initial as unknown as Row | undefined} onClose={onClose} onSubmit={save} />;
}
...
export function GuardrailsPage() {
  ...
  return <><PageHeader eyebrow="Compliance" title="Guardrails" description="Configure reusable DLP/AV policies, understand their runtime coverage and compare enabled policies without calling a model." />
    ...
    <section className="notice-card guardrail-policy-boundary"><h2>Gateway-native guardrail policies</h2><p>Policies select independently operated DLP and antivirus services. They do not store scanner credentials or provider-specific executable code. Direct deployment policies and matching scoped attachments are combined and fail closed when a required scanner is unavailable.</p></section>
    ...
    {editing !== undefined && <GuardrailPolicyForm initial={editing || undefined} client={client} onClose={() => setEditing(undefined)} onSaved={() => load()} />}
