...
type GroupedRequestLog = Row & {
  ...
  total_tokens: number; training_tokens: number; input_characters: number; input_pages: number; input_audio_milliseconds: number; tool_requests: number; cache_read_input_tokens: number; cache_write_input_tokens: number; search_requests: number; cache_hits: number; latency_ms: number; cost: number; currency: string; started_at: string; ended_at: string;
};
...
export function groupRequestLogs(rows: RequestLog[], field: "session_id" | "trace_id"): GroupedRequestLog[] {
  ...
  for (const row of rows) {
    ...
    const current = groups.get(key) || {
      id: key, group_id: groupID, requests: 0, errors: 0, models: [], providers: [], total_tokens: 0, training_tokens: 0, input_characters: 0, input_pages: 0, input_audio_milliseconds: 0, tool_requests: 0,
...
export function RequestLogsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const columns = [
    ...
    { key: "total_tokens", label: "Tokens" }, { key: "training_tokens", label: "Training tokens" }, { key: "input_characters", label: "Characters" }, { key: "input_pages", label: "Pages" }, { key: "input_audio_milliseconds", label: "Audio (ms)" }, { key: "tool_requests", label: "Tool calls" },
