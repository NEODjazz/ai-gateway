...
export function AuditLogsPage() {
  ...
  return <>{error && <ErrorState message={error} retry={() => void load()} />}<ManagedDataTable rows={rows} columns={columns} rowKey="id" defaultHidden={["request_id", "actor_credential_id", "details"]} toolbarExtra={<ToolbarIconButton icon="filter" label="Filter" active={Boolean(filters.actor_id || filters.action)} onClick={() => setFilterOpen(true)} />} onRefresh={load} searchPlaceholder="Search audit logs" actions={(row) => <ActionsMenu label={`Actions for audit event ${String(row.id)}`} items={[{ label: "Details", onSelect: () => setDetail(row as AuditEvent) }]} />} />{filterOpen && <ModalFrame label="Filter audit logs" onClose={() => setFilterOpen(false)}><form className="modal compact-modal"    onSubmit={apply}><div className="modal-heading"><h2>Filter audit logs</h2><button type="button" className="icon-button" aria-label="Close filters" onClick={() => setFilterOpen(false)}>×</button></div><div className="form-grid"><label>Actor ID<input value={draft.actor_id} onChange={(event) => setDraft({ ...draft, actor_id: event.target.value })} /></label><label>Action<input value={draft.action} onChange={(event) => setDraft({ ...draft, action: event.target.value })} /></label></div><div className="modal-actions"><button type="button" className="secondary" onClick={() => setDraft({ actor_id: "", action: "" })}>Reset filters</button><button>Apply filters</button></div></form></ModalFrame>}{detail && <ModalFrame label="Audit log details" onClose={() => setDetail(undefined)}><section className="modal"><div className="modal-heading"><h2>Audit log details</h2><button className="icon-button" aria-label="Close audit details" onClick={() => setDetail(undefined)}>×</button></div><dl className="detail-grid"><div><dt>Event</dt><dd>{detail.id}</dd></div><div><dt>Time</dt><dd>{formatTimestamp(detail.occurred_at)}</dd></div><div><dt>Action</dt><dd>{detail.action}</dd></div><div><dt>Outcome</dt><dd>{detail.outcome}</dd></div><div><dt>Target</dt><dd>{detail.target_type} · {detail.target_id || "—"}</dd></div><div><dt>Actor</dt><dd>{detail.actor_id || "—"}</dd></div><div><dt>Request</dt><dd>{detail.request_id || "—"}</dd></div><div><dt>Credential</dt><dd>{detail.actor_credential_id || "—"}</dd></div></dl><h3>Metadata</h3><pre>{JSON.stringify(detail.details || {}, null, 2)}</pre></section></ModalFrame>}</>;
}
