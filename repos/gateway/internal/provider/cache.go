...
func providerCacheKey(kind string, req modules.RequestContext) string {
	if kind == "chat" && (req.Request.WebSearchOptions != nil || req.Request.WebFetchOptions != nil || openai.ChatRequestsAudio(req.Request) || len(req.Request.BedrockRequestMetadata) > 0 || req.Request.BedrockGuardrailConfig != nil || len(req.Request.GeminiSafetySettings) > 0) {
		return ""
	}
