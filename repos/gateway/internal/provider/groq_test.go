...
func TestGroqStreamsResponsesWithUsage(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, "data: {\"type\":\"response.output_text.delta\",\"delta\":\"ok\"}\n\ndata: {\"type\":\"response.completed\",\"response\":{\"id\":\"response\",\"object\":\"response\",\"model\":\"model\",\"status\":\"completed\",\"usage\":{\"input_tokens\":2,\"output_tokens\":1,\"output_tokens_details\":{\"cached_tokens\":1},\"total_tokens\":3}}}\n\n")
	}))
	defer server.Close()
	...
	if err != nil || response.Usage.TotalTokens != 3 || response.OutputText != "ok" || response.Usage.OutputTokensDetails == nil || response.Usage.OutputTokensDetails.CachedTokens != 1 || len(events) != 2 {
