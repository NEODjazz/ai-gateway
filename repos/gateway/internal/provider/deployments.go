...
func validDeploymentCapabilities(capabilities []string) bool {
	...
	if seen["custom_tools"] && (!seen["responses"] || !seen["tools"]) {
		return false
	}
...
func ValidModelCapability(capability string) bool {
	...
	case "chat", "responses", "interactions", "interaction_agents", "interaction_environment_reuse", "gemini_safety_settings", "background_interactions", "embeddings", "rerank", "moderation",
		...
		"web_search", "web_fetch", "tool_search", "memory_tool", "bash_tool", "text_editor_tool", "computer_toolset", "browser_toolset", "thinking", "zero_output", "inference_geo", "context_management", "tool_result_error", "document_citations", "document_metadata", "document_text", "audio", "audio_input", "video_input", "prompt_cache", "assistant_prefill", "background_responses", "file_input", "bedrock_invoke", "gemini_code_execution", "gemini_audio_timestamp", "gemini_media_resolution", "url_context", "google_maps":
		return true
	default:
		return false
	}
...
func supportsManagedAdapterCapability(endpoint Endpoint, capability string) bool {
	...
	case "custom_tools":
		client, ok := endpoint.Provider.(interface{ SupportsResponseCustomTools() bool })
		return ok && client.SupportsResponseCustomTools()
	...
	case "response_shell":
		client, ok := endpoint.Provider.(interface{ SupportsResponseShell() bool })
		return ok && client.SupportsResponseShell()
	case "response_apply_patch":
		client, ok := endpoint.Provider.(interface{ SupportsResponseApplyPatch() bool })
		return ok && client.SupportsResponseApplyPatch()
