...
func validDeploymentCapabilities(capabilities []string) bool {
	allowed := map[string]bool{
		...
		"stream": true, "tools": true, "structured_output": true, "mcp": true, "vision": true, "web_search": true, "web_fetch": true, "search": true, "audio": true, "prompt_cache": true, "image_generation": true, "image_edit": true, "image_variation": true, "audio_transcription": true, "audio_speech": true, "ocr": true,
	}
...
func (r *Router) endpointForDeployment(deployment ModelDeployment) (Endpoint, error) {
	...
	client := providerFor(config.ProviderEndpointConfig{Type: managed.Type, BaseURL: managed.BaseURL, APIKey: secret, Stream: hasCapability(deployment.Capabilities, "stream"), APIVersion: managed.APIVersion, AuthType: managed.AuthType})
	if client == nil {
		return Endpoint{}, ErrInvalidDeployment
	}
