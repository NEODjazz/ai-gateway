...
func semanticRequest(req modules.RequestContext, endpoint Endpoint) (string, string, bool) {
	request := req.Request
	if count, message := openai.ChatRequestPromptCacheBreakpoints(request); message != "" || count > 0 {
		return "", "", false
	}
	...
	for _, message := range request.Messages {
		if len(message.ToolCalls) > 0 || message.ToolCallID != "" || message.ToolResultError || len(message.AnthropicDocumentCitations) > 0 || len(message.AnthropicDocumentMetadata) > 0 || message.Audio != nil || message.ReasoningContent != "" || len(message.Reasoning) > 0 || len(message.NativeContent) > 0 || !textOnlyContent(message.Content) {
			return "", "", false
		}
