...
func validateChatAdapter(client Client, request openai.ChatCompletionRequest) error {
	if request.BedrockServiceTier != "" || request.BedrockPerformanceLatency != "" || len(request.BedrockAdditionalModelResponseFieldPaths) > 0 || len(request.BedrockRequestMetadata) > 0 {
