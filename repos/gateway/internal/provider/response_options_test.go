...
func TestRouterRejectsInvalidResponseOptionsBeforeRouting(t *testing.T) {
	...
	obfuscation := false
	for _, request := range []openai.ResponseRequest{{TopLogprobs: &badCount}, {Truncation: &badTruncation}, {SafetyIdentifier: strings.Repeat("я", 65)}, {ServiceTier: "unknown"}, {Text: map[string]any{"verbosity": "unknown"}}, {PromptCacheOptions: &openai.PromptCacheOptions{Mode: "automatic"}}, {PromptCacheOptions: &openai.PromptCacheOptions{TTL: "1h"}}, {PromptCacheOptions: &openai.PromptCacheOptions{ComparisonResponseID: strings.Repeat("r", 257)}}, {PromptCacheRetention: "forever"}, {StreamOptions: &openai.ResponseStreamOptions{IncludeObfuscation: &obfuscation}}} {
