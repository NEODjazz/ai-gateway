...
func TestResponsePreservesEncryptedReasoning(t *testing.T) {
	const item = `{"id":"reason","type":"reasoning","encrypted_content":"opaque-example+/=","summary":[]}`
	...
	for name, decode := range decoders {
		t.Run(name, func(t *testing.T) {
			...
			if len(output.Output) != 1 || output.Output[0]["encrypted_content"] != "opaque-example+/=" {
				t.Fatal("encrypted reasoning was lost")
			}
		})
	}
}
