...
func normalizeOnboardingCatalog(input modelcatalog.Catalog) (modelcatalog.Catalog, error) {
	...
	allowedCapabilities := map[string]bool{"chat": true, "responses": true, "embeddings": true, "rerank": true, "moderation": true, "stream": true, "tools": true, "structured_output": true, "mcp": true, "vision": true, "web_search": true, "audio": true, "prompt_cache": true}
	for _, entry := range catalog.Models {
		...
		if entry.Currency != "" && (len(entry.Currency) != 3 || entry.Currency != strings.ToUpper(entry.Currency)) || (entry.InputCostPer1M > 0 || entry.OutputCostPer1M > 0 || entry.SearchCostPer1K > 0 || entry.CharacterCostPer1M > 0 || entry.PageCostPer1K > 0) && entry.Currency == "" {
			return modelcatalog.Catalog{}, ErrInvalidModelOnboarding
		}
	}
	return catalog, nil
