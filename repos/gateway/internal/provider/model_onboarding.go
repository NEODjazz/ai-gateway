...
func normalizeOnboardingCatalog(input modelcatalog.Catalog) (modelcatalog.Catalog, error) {
	...
	for _, entry := range catalog.Models {
		...
		if entry.Currency != "" && (len(entry.Currency) != 3 || entry.Currency != strings.ToUpper(entry.Currency)) || (entry.InputCostPer1M > 0 || entry.OutputCostPer1M > 0 || entry.SearchCostPer1K > 0 || entry.CharacterCostPer1M > 0) && entry.Currency == "" {
			return modelcatalog.Catalog{}, ErrInvalidModelOnboarding
		}
	}
	return catalog, nil
