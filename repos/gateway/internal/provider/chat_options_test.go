...
func TestCompatibleChatGenerationOptionsRoundTrip(t *testing.T) {
	for _, streaming := range []bool{false, true} {
		t.Run(fmt.Sprint(streaming), func(t *testing.T) {
			...
			if err := json.Unmarshal([]byte(`{"model":"test","reasoning_effort":"high","n":2,"safety_identifier":"hashed-user","logprobs":true,"top_logprobs":0,"frequency_penalty":0,"presence_penalty":-1,"logit_bias":{"10":-100}}`), &request); err != nil {
				t.Fatal(err)
			}
			...
			for name, want := range map[string]string{"reasoning_effort": `"high"`, "n": "2", "safety_identifier": `"hashed-user"`, "logprobs": "true", "top_logprobs": "0", "frequency_penalty": "0", "presence_penalty": "-1", "logit_bias": `{"10":-100}`} {
