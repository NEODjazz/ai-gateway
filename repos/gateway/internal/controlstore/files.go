...
func (s *PostgresStore) Create(ctx context.Context, file filestate.File, ownerQuota int64) (filestate.File, error) {
	...
	if _, err = tx.Exec(ctx, `DELETE FROM gateway_files WHERE owner_key=$1 AND expires_at IS NOT NULL AND expires_at<=now()`, file.OwnerKey); err != nil {
		return filestate.File{}, err
	}
	var used int64
	...
	command, err := tx.Exec(ctx, `INSERT INTO gateway_files (id,owner_key,filename,purpose,content_type,bytes,content,expires_at)
		VALUES ($1,$2,$3,$4,$5,$6,$7,CASE WHEN $8::bigint>0 THEN now()+make_interval(secs=>$8) ELSE NULL END)
		ON CONFLICT DO NOTHING`, file.ID, file.OwnerKey, file.Filename, file.Purpose, file.ContentType, file.Bytes, file.Content, file.ExpiresAfterSeconds)
	if err != nil {
		return filestate.File{}, err
	}
	...
	if err = tx.QueryRow(ctx, `SELECT created_at,expires_at FROM gateway_files WHERE owner_key=$1 AND id=$2`, file.OwnerKey, file.ID).Scan(&file.CreatedAt, &file.ExpiresAt); err != nil {
		return filestate.File{}, err
	}
...
func (s *PostgresStore) List(ctx context.Context, owner, purpose string, limit int, after string) ([]filestate.File, string, error) {
	...
	if _, err := s.pool.Exec(ctx, `DELETE FROM gateway_files WHERE owner_key=$1 AND expires_at IS NOT NULL AND expires_at<=now()`, owner); err != nil {
		return nil, "", err
	}
	...
	rows, err := s.pool.Query(ctx, `SELECT id,filename,purpose,content_type,bytes,created_at,expires_at
		FROM gateway_files
	...
	for rows.Next() {
		...
		file.OwnerKey = owner
		if err := rows.Scan(&file.ID, &file.Filename, &file.Purpose, &file.ContentType, &file.Bytes, &file.CreatedAt, &file.ExpiresAt); err != nil {
			return nil, "", err
		}
...
func (s *PostgresStore) Get(ctx context.Context, owner, id string, content bool) (filestate.File, error) {
	...
	var payload []byte
	_, err := s.pool.Exec(ctx, `DELETE FROM gateway_files WHERE owner_key=$1 AND id=$2 AND expires_at IS NOT NULL AND expires_at<=now()`, owner, id)
	if err != nil {
		return filestate.File{}, err
	}
	err = s.pool.QueryRow(ctx, `SELECT id,filename,purpose,content_type,bytes,created_at,expires_at,CASE WHEN $3 THEN content ELSE NULL END FROM gateway_files WHERE owner_key=$1 AND id=$2`, owner, id, content).Scan(&f.ID, &f.Filename, &f.Purpose, &f.ContentType, &f.Bytes, &f.CreatedAt, &f.ExpiresAt, &payload)
	if errors.Is(err, pgx.ErrNoRows) {
...
func (s *PostgresStore) Delete(ctx context.Context, owner, id string) error {
	...
	if _, err := s.pool.Exec(ctx, `DELETE FROM gateway_files WHERE owner_key=$1 AND id=$2 AND expires_at IS NOT NULL AND expires_at<=now()`, owner, id); err != nil {
		return err
	}
