...
func prepareConversationStore(t *testing.T) *PostgresStore {
	...
	_, err = store.pool.Exec(t.Context(), `CREATE TABLE gateway_conversations (
		...
		active_execution_id TEXT,lease_until TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT now(),updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),PRIMARY KEY(owner_key,id));
