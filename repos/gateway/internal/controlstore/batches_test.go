...
func TestPostgresBatchLifecycleIsAtomicAndOwnerIsolatedIntegration(t *testing.T) {
	...
	batch := batchstate.Batch{ID: "batch_a", OwnerKey: "owner-a", InputFileID: file.ID, Endpoint: "/v1/chat/completions", CompletionWindow: "24h", Status: "queued", Metadata: []byte(`{}`), Identity: []byte(`{"credential_id":"credential"}`), Total: 2, ExpiresAt: time.Now().Add(24 * time.Hour)}
	...
	if created.Total != 2 || created.Status != "queued" {
