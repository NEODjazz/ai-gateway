...
func Parse(raw string) (Catalog, error) {
	...
	for index, model := range catalog.Models {
		...
		if model.MaxInputTokens < 0 || model.MaxOutputTokens < 0 || model.InputCostPer1M < 0 || model.OutputCostPer1M < 0 || model.TrainingCostPer1M < 0 || model.SearchCostPer1K < 0 || model.CharacterCostPer1M < 0 || model.PageCostPer1K < 0 || model.AudioCostPerMinute < 0 || model.VideoCostPerSecond < 0 {
