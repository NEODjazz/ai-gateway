...
func TestAdminRequestLogsRequireAdminAndValidateFilters(t *testing.T) {
	client := &recordingRequestLogClient{}
	handler := Routes(NewHandler(modulesPipeline("admin"), modelsProvider{}).WithRequestLogs(client))
	request := httptest.NewRequest(http.MethodGet, "/admin/v1/request-logs?days=14&limit=25&status=error&team_id=team-a&organization_id=org-a&session_id=session-1&trace_id=0123456789abcdef0123456789abcdef&tag=production&cache_status=hit", nil)
	request.Header.Set("Authorization", "Bearer secret")
	request.Header.Set("X-Request-ID", "req-admin")
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusOK || client.filter.Days != 14 || client.filter.Limit != 25 || client.filter.Status != "error" || client.filter.TeamID != "team-a" || client.filter.OrganizationID != "org-a" || client.filter.SessionID != "session-1" || client.filter.TraceID != "0123456789abcdef0123456789abcdef" || client.filter.Tag != "production" || client.filter.CacheStatus != "hit" || client.audit.RequestID != "req-admin" {
...
func TestRemoteRequestLogsUseScopedManagementCredential(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Authorization") != "" || r.Header.Get("X-Management-Token") != "billing-secret" || r.URL.Query().Get("team_id") != "team-a" || r.URL.Query().Get("organization_id") != "org-a" || r.URL.Query().Get("trace_id") != "trace-1" || r.URL.Query().Get("tag") != "production" || r.URL.Query().Get("cache_status") != "miss" || r.URL.Query().Get("limit") != "20" {
