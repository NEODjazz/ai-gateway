...
func (c *recordingRequestLogClient) ListRequestLogs(_ context.Context, audit ManagementAudit, filter RequestLogFilter) (RequestLogPage, error) {
	...
	return RequestLogPage{Data: []RequestLog{{RequestID: "req-1", Timestamp: "2026-08-25T12:00:00Z", OrganizationID: "org-a", Status: "ok", APIType: "chat_completions", Phase: "commit", FirstTokenLatencyMS: 42, RetryCount: 2, FallbackCount: 1, CacheStatus: "hit", CacheKind: "semantic", UsageEstimated: true, Currency: "USD"}}}, nil
}
...
func TestAdminRequestLogsRequireAdminAndValidateFilters(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "/admin/v1/request-logs?days=14&limit=25&status=error&team_id=team-a&organization_id=org-a&session_id=session-1&cache_status=hit", nil)
	request.Header.Set("Authorization", "Bearer secret")
	request.Header.Set("X-Request-ID", "req-admin")
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusOK || client.filter.Days != 14 || client.filter.Limit != 25 || client.filter.Status != "error" || client.filter.TeamID != "team-a" || client.filter.OrganizationID != "org-a" || client.filter.SessionID != "session-1" || client.filter.CacheStatus != "hit" || client.audit.RequestID != "req-admin" {
		t.Fatalf("status=%d filter=%+v audit=%+v body=%s", response.Code, client.filter, client.audit, response.Body.String())
	}
	for _, expected := range []string{`"organization_id":"org-a"`, `"first_token_latency_ms":42`, `"retry_count":2`, `"fallback_count":1`, `"cache_kind":"semantic"`, `"usage_estimated":true`} {
...
func TestRemoteRequestLogsUseScopedManagementCredential(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Authorization") != "" || r.Header.Get("X-Management-Token") != "billing-secret" || r.URL.Query().Get("team_id") != "team-a" || r.URL.Query().Get("organization_id") != "org-a" || r.URL.Query().Get("cache_status") != "miss" || r.URL.Query().Get("limit") != "20" {
	...
	page, err := client.ListRequestLogs(context.Background(), ManagementAudit{RequestID: "admin-request", ActorID: "admin", CredentialID: "cred"}, RequestLogFilter{Days: 7, Limit: 20, TeamID: "team-a", OrganizationID: "org-a", CacheStatus: "miss"})
