...
func TestAdminRequestLogsRequireAdminAndValidateFilters(t *testing.T) {
	client := &recordingRequestLogClient{}
	handler := Routes(NewHandler(modulesPipeline("admin"), modelsProvider{}).WithRequestLogs(client))
	request := httptest.NewRequest(http.MethodGet, "/admin/v1/request-logs?days=14&from=2026-08-01T00%3A00%3A00Z&to=2026-08-02T00%3A00%3A00Z&limit=25&status=error&team_id=team-a&organization_id=org-a&session_id=session-1&trace_id=0123456789abcdef0123456789abcdef&tag=production&cache_status=hit&failure_class=upstream&min_cost=0.01&max_cost=1.5", nil)
	request.Header.Set("Authorization", "Bearer secret")
	...
	handler.ServeHTTP(invalidRange, httptest.NewRequest(http.MethodGet, "/admin/v1/request-logs?from=2026-08-01T00%3A00%3A00Z", nil))
	if invalidRange.Code != http.StatusBadRequest {
		t.Fatalf("partial custom range accepted: %d", invalidRange.Code)
	}
}
...
func TestRemoteRequestLogsUseScopedManagementCredential(t *testing.T) {
	...
	page, err := client.ListRequestLogs(context.Background(), ManagementAudit{RequestID: "admin-request", ActorID: "admin", CredentialID: "cred"}, RequestLogFilter{Days: 7, From: time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC), To: time.Date(2026, 8, 2, 0, 0, 0, 0, time.UTC), Limit: 20, TeamID: "team-a", OrganizationID: "org-a", TraceID: "trace-1", Tag: "production", CacheStatus: "miss", FailureClass: "upstream", MinCost: &minCost, MaxCost: &maxCost})
