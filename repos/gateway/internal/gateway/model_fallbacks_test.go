...
func TestPrepareModelFallbacksIntersectsKeyAndTagModelGrants(t *testing.T) {
	...
	handler := NewHandler(modules.NewPipeline(nil), fallbackResolverProvider{targets: []string{"allowed-fallback", "key-denied", "tag-denied"}}).WithAccessRegistry(registry)
	req := modules.RequestContext{AllowedModels: []string{"primary", "allowed-fallback", "tag-denied"}, Tags: []string{"regulated"}}
