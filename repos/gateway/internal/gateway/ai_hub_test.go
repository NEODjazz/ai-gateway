...
func TestAIHubAndCostRecommendations(t *testing.T) {
	catalog, _ := modelcatalog.Parse(`{"version":"test","models":[{"provider":"demo-a","model":"m1","input_cost_per_1m":2,"output_cost_per_1m":2,"search_cost_per_1k":10,"currency":"USD"},{"provider":"cheap-a","model":"m1","input_cost_per_1m":1,"output_cost_per_1m":1,"currency":"USD"},{"provider":"demo-a","model":"m2"}]}`)
	...
	if hub.Code != http.StatusOK || strings.Contains(hub.Body.String(), "base_url") || !strings.Contains(hub.Body.String(), `"available":true`) || !strings.Contains(hub.Body.String(), `"search_cost_per_1k":10`) {
