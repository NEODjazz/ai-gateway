...
func TestAdminUIServesEmbeddedSameOriginAssets(t *testing.T) {
	...
	for _, expected := range []string{"AI Gateway Console", "/ui/assets/app.css", "/ui/assets/app.js", "Admin bearer token", "Overview", "Models", "Budgets", "Audit log"} {
	...
	for path, contentType := range map[string]string{"/ui/assets/app.css": "text/css; charset=utf-8", "/ui/assets/app.js": "text/javascript; charset=utf-8"} {
		...
		assertAdminUISecurityHeaders(t, asset.Header(), "public, max-age=31536000, immutable")
	}
}
