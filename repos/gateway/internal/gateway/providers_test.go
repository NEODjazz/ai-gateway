...
func TestAdminListsProviderCapabilityProfiles(t *testing.T) {
	...
	if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), `"type":"voyage","operations":["embeddings","rerank"],"capabilities":["embeddings","rerank"],"auth_types":[],"chat_parameters":{"reasoning_effort":[],"logprobs":[],"service_tier":[]}`) || !strings.Contains(response.Body.String(), `"web_fetch"`) || !strings.Contains(response.Body.String(), `"auth_types":["api_key","gcp_adc"]`) || !strings.Contains(response.Body.String(), `"count_tokens"`) || !strings.Contains(response.Body.String(), `"service_tier":["auto","standard_only"]`) {
