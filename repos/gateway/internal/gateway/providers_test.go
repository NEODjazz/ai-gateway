...
func TestAdminListsProviderCapabilityProfiles(t *testing.T) {
	...
	if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), `"type":"voyage","operations":["embeddings","rerank"],"capabilities":["embeddings","rerank"],"auth_types":[],"chat_parameters":{"supported_options":[],"reasoning_effort":[],"reasoning_format":[],"citation_options":[],"thinking":[],"logprobs":[],"service_tier":[]},"response_parameters":{"supported_options":[],"reasoning_effort":[],"service_tier":[]}`) || !strings.Contains(response.Body.String(), `"web_fetch"`) || !strings.Contains(response.Body.String(), `"auth_types":["api_key","gcp_adc"]`) || !strings.Contains(response.Body.String(), `"count_tokens"`) || !strings.Contains(response.Body.String(), `"service_tier":["auto","standard_only"]`) || !strings.Contains(response.Body.String(), `"supported_options":["metadata","modalities","audio"`) || !strings.Contains(response.Body.String(), `"response_parameters":{"supported_options":["max_output_tokens","max_tokens","temperature","top_p","reasoning"]`) {
