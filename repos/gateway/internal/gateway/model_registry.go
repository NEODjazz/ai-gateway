...
func (h Handler) PutModelCatalog(w http.ResponseWriter, r *http.Request) {
	...
	for _, entry := range catalog.Models {
		...
		if (entry.InputCostPer1M > 0 || entry.OutputCostPer1M > 0 || entry.SearchCostPer1K > 0 || entry.CharacterCostPer1M > 0) && entry.Currency == "" {
