...
func TestChatGenerationOptionValidation(t *testing.T) {
	for _, tc := range []struct {
		...
		{`{"prompt_cache_options":{"prewarm":true}}`, false},
		...
		{`{"citation_options":"disabled"}`, true},
		{`{"citation_options":"unexpected"}`, false},
		{`{"thinking":{"type":"enabled"}}`, true},
		{`{"thinking":{"type":"disabled"}}`, true},
		{`{"thinking":{"type":"unexpected"}}`, false},
		{`{"include_reasoning":false}`, true},
		{`{"reasoning_format":"hidden"}`, true},
		{`{"reasoning_format":"raw"}`, true},
		{`{"reasoning_format":"parsed"}`, true},
		{`{"reasoning_format":"unexpected"}`, false},
		{`{"include_reasoning":true,"reasoning_format":"parsed"}`, false},
