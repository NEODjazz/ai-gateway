...
func main() {
	...
	if providerControlStore != nil {
		handler = handler.WithMCPCallStore(providerControlStore).WithFileStore(providerControlStore, gateway.FileRuntimeConfig{MaxBytes: cfg.Files.MaxBytes, OwnerQuotaBytes: cfg.Files.OwnerQuotaBytes}).WithSkillStore(providerControlStore)
	}
